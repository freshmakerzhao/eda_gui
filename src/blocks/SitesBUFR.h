...
public:
    SitesBUFR(const QColor &color, int tile_index_x, int tile_index_y, std::string &cur_name, int site_index)
            :SitesBlock(color, GLOBAL_SITE_BLOCK_WIDTH-400, GLOBAL_SITE_BLOCK_HEIGHT-650, tile_index_x, tile_index_y, "BUFR", cur_name, site_index) {}
};
//class SitesBUFR : public SitesBlock
//{
//public:
//    // 颜色、名称中的坐标，宽度、高度、类型、在当前tile中的索引
//    SitesBUFR(const QColor &color, int cur_width, int cur_height, int tile_index_x,int tile_index_y,std::string &cur_type, std::string &cur_name, int site_index);
