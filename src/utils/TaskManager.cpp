...
void TaskManager::handleMessage(ProcessMessage &msg) {
    ...
    QSettings settings(projectPath + "/runs/.works/.state", QSettings::IniFormat);
    if (msg.phase == "Synthesis") {
        settings.setValue(FlowPhase::Synthsis, FlowState::Fail);
    }
    if (msg.phase == "Implementation") {
        settings.setValue(FlowPhase::Implementation, FlowState::Fail);
    }
    if (msg.phase == "Generate Bitstream"){
        settings.setValue(FlowPhase::GenerateBitstream, FlowState::Fail);
    }
