...
// The #defines in Scintilla.h and the enums in exsciscintillabase.h conflict
...
// The ctor.
ExsciScintillaBase::ExsciScintillaBase(QWidget *parent)
...
// The dtor.
ExsciScintillaBase::~ExsciScintillaBase()
{
    // The ExsciScintillaQt object isn't a child so delete it explicitly.
    delete sci;
...
ExsciScintillaBase *ExsciScintillaBase::pool()
{
...
// itself.
void ExsciScintillaBase::setScrollBars()
{
    sci->SetScrollBars();
}
...
// API.
long ExsciScintillaBase::SendScintilla(unsigned int msg, unsigned long wParam,
        long lParam) const
{
...
// Overloaded message send.
long ExsciScintillaBase::SendScintilla(unsigned int msg,
        const char *lParam) const
{
...
// Overloaded message send.
long ExsciScintillaBase::SendScintilla(unsigned int msg, const char *wParam,
        const char *lParam) const
{
...
// Overloaded message send.
long ExsciScintillaBase::SendScintilla(unsigned int msg, long wParam) const
{
...
// Overloaded message send.
long ExsciScintillaBase::SendScintilla(unsigned int msg, int wParam) const
{
    return sci->WndProc(msg, static_cast<uptr_t>(wParam),
            static_cast<sptr_t>(0));
}
...
// Overloaded message send.
long ExsciScintillaBase::SendScintilla(unsigned int msg, long cpMin, long cpMax,
...
// Overloaded message send.
long ExsciScintillaBase::SendScintilla(unsigned int msg, const QColor &col) const
{
        ...
        QPainter *hdc, const QRect &rc, long cpMin, long cpMax) const
{
    ...
    rf.hdc = rf.hdcTarget = reinterpret_cast<EXSCI_SCI_NAMESPACE(SurfaceID)>(hdc);
...
void *ExsciScintillaBase::SendScintillaPtrResult(unsigned int msg) const
{
...
void ExsciScintillaBase::contextMenuEvent(QContextMenuEvent *e)
{
    sci->ContextMenu(EXSCI_SCI_NAMESPACE(Point)(e->globalX(), e->globalY()));
}


// Re-implemented to tell the widget it has the focus.
void ExsciScintillaBase::focusInEvent(QFocusEvent *e)
{
...
void ExsciScintillaBase::focusOutEvent(QFocusEvent *e)
{
    if (e->reason() == Qt::ActiveWindowFocusReason)
    {
        ...
        if (!aw || aw->parent() != this || !aw->inherits("ExsciSciListBox"))
...
bool ExsciScintillaBase::focusNextPrevChild(bool next)
{
...
void ExsciScintillaBase::handleSelection()
{
...
// Handle key presses.
void ExsciScintillaBase::keyPressEvent(QKeyEvent *e)
{
    int modifiers = 0;
...
int ExsciScintillaBase::commandKey(int qt_key, int &modifiers)
{
    int key;
...
ExsciScintillaBase::ScintillaBytes ExsciScintillaBase::textAsBytes(const QString &text) const
{
...
QString ExsciScintillaBase::bytesAsText(const char *bytes) const
{
    if (sci->IsUnicodeMode())
...
void ExsciScintillaBase::mouseDoubleClickEvent(QMouseEvent *e)
{
    ...
    unsigned clickTime = sci->lastClickTime + EXSCI_SCI_NAMESPACE(Platform)::DoubleClickTime() - 1;

    bool shift = e->modifiers() & Qt::ShiftModifier;
    bool ctrl = e->modifiers() & Qt::ControlModifier;
    bool alt = e->modifiers() & Qt::AltModifier;

    sci->ButtonDown(EXSCI_SCI_NAMESPACE(Point)(e->x(), e->y()), clickTime,
            shift, ctrl, alt);
...
// Handle a mouse move.
void ExsciScintillaBase::mouseMoveEvent(QMouseEvent *e)
{
    sci->ButtonMove(EXSCI_SCI_NAMESPACE(Point)(e->x(), e->y()));
}


// Handle a mouse button press.
void ExsciScintillaBase::mousePressEvent(QMouseEvent *e)
{
    setFocus();

    EXSCI_SCI_NAMESPACE(Point) pt(e->x(), e->y());
    ...
    if (e->button() == Qt::LeftButton)
    {
        ...
        if (triple_click.isActive() && (e->globalPos() - triple_click_at).manhattanLength() < QApplication::startDragDistance())
            clickTime = sci->lastClickTime + EXSCI_SCI_NAMESPACE(Platform)::DoubleClickTime() - 1;
        else
            clickTime = sci->lastClickTime + EXSCI_SCI_NAMESPACE(Platform)::DoubleClickTime() + 1;
...
void ExsciScintillaBase::mouseReleaseEvent(QMouseEvent *e)
{
    if (e->button() != Qt::LeftButton)
        return;
...
// Handle paint events.
void ExsciScintillaBase::paintEvent(QPaintEvent *e)
{
    sci->paintEvent(e);
}
...
void ExsciScintillaBase::resizeEvent(QResizeEvent *)
{
    sci->ChangeSize();
}
...
void ExsciScintillaBase::scrollContentsBy(int, int)
{
}


// Handle the vertical scrollbar.
void ExsciScintillaBase::handleVSb(int value)
{
    sci->ScrollTo(value);
}
...
void ExsciScintillaBase::handleHSb(int value)
{
...
// Handle drag enters.
void ExsciScintillaBase::dragEnterEvent(QDragEnterEvent *e)
{
    ExsciScintillaBase::dragMoveEvent(e);
}


// Handle drag leaves.
void ExsciScintillaBase::dragLeaveEvent(QDragLeaveEvent *)
{
    sci->SetDragPosition(EXSCI_SCI_NAMESPACE(SelectionPosition)());
}


// Handle drag moves.
void ExsciScintillaBase::dragMoveEvent(QDragMoveEvent *e)
{
    sci->SetDragPosition(
            sci->SPositionFromLocation(
                    EXSCI_SCI_NAMESPACE(Point)(e->pos().x(), e->pos().y()),
...
// Handle drops.
void ExsciScintillaBase::dropEvent(QDropEvent *e)
{
    bool moving;
    int len;
    const char *s;
    ...
    std::string dest = EXSCI_SCI_NAMESPACE(Document)::TransformLineEnds(s, len,
                sci->pdoc->eolMode);
...
void ExsciScintillaBase::acceptAction(QDropEvent *e)
{
...
bool ExsciScintillaBase::canInsertFromMimeData(const QMimeData *source) const
{
...
QByteArray ExsciScintillaBase::fromMimeData(const QMimeData *source, bool &rectangular) const
{
...
QMimeData *ExsciScintillaBase::toMimeData(const QByteArray &text, bool rectangular) const
{
...
void ExsciScintillaBase::connectVerticalScrollBar()
{
...
void ExsciScintillaBase::connectHorizontalScrollBar()
{
...
void ExsciScintillaBase::replaceVerticalScrollBar(QScrollBar *scrollBar)
{
...
void ExsciScintillaBase::replaceHorizontalScrollBar(QScrollBar *scrollBar)
{
