// This module implements the ExsciMacro class.



#include "Exsci/exscimacro.h"
...
// The ctor.
ExsciMacro::ExsciMacro(ExsciScintilla *parent)
    : QObject(parent), exsci(parent)
{
}


// The ctor that initialises the macro.
ExsciMacro::ExsciMacro(const QString &asc, ExsciScintilla *parent)
    : QObject(parent), exsci(parent)
{
    load(asc);
}
...
// The dtor.
ExsciMacro::~ExsciMacro()
{
}


// Clear the contents of the macro.
void ExsciMacro::clear()
{
    macro.clear();
}
...
bool ExsciMacro::load(const QString &asc)
{
    bool ok = true;
...
void ExsciMacro::play()
{
    ...
    for (it = macro.begin(); it != macro.end(); ++it)
        exsci->SendScintilla((*it).msg, (*it).wParam, (*it).text.data());
}


// Start recording.
void ExsciMacro::startRecording()
{
    if (!exsci)
        return;
    ...
    connect(exsci, SIGNAL(SCN_MACRORECORD(unsigned int, unsigned long, void *)),
            SLOT(record(unsigned int, unsigned long, void *)));

    exsci->SendScintilla(ExsciScintillaBase::SCI_STARTRECORD);
}


// End recording.
void ExsciMacro::endRecording()
{
    if (!exsci)
        return;

    exsci->SendScintilla(ExsciScintillaBase::SCI_STOPRECORD);
    exsci->disconnect(this);
}


// Record a command.
void ExsciMacro::record(unsigned int msg, unsigned long wParam, void *lParam)
{
    Macro m;
    ...
    switch (msg)
    {
    case ExsciScintillaBase::SCI_ADDTEXT:
        m.text = QByteArray(reinterpret_cast<const char *>(lParam), wParam);
        break;

    case ExsciScintillaBase::SCI_REPLACESEL:
        if (!macro.isEmpty() && macro.last().msg == ExsciScintillaBase::SCI_REPLACESEL)
        {
    ...
    case ExsciScintillaBase::SCI_INSERTTEXT:
    case ExsciScintillaBase::SCI_APPENDTEXT:
    case ExsciScintillaBase::SCI_SEARCHNEXT:
    case ExsciScintillaBase::SCI_SEARCHPREV:
