// This defines the interface to the ExsciLexer class.
...
class ExsciAbstractAPIs;
class ExsciScintilla;


//! \brief The ExsciLexer class is an abstract class used as a base for language
//! lexers.
//!
...
//!
//! ExsciLexer provides convenience methods for saving and restoring user
//! preferences for fonts and colours.
//!
//! If you want to write a lexer for a new language then you can add it to the
//! underlying Scintilla code and implement a corresponding ExsciLexer sub-class
//! to manage the different styles used.  Alternatively you can implement a
//! sub-class of ExsciLexerCustom.
class EXSCINTILLA_EXPORT ExsciLexer : public QObject
{
    Q_OBJECT

public:
    //! Construct a ExsciLexer with parent \a parent.  \a parent is typically
    //! the ExsciScintilla instance.
    ExsciLexer(QObject *parent = 0);

    //! Destroy the EXSciLexer.
    virtual ~ExsciLexer();
    ...
    //! Returns the identifier (i.e. a ExsciScintillaBase::SCLEX_* value) of the
    //! lexer.  This is only used if lexer() returns 0.  The default
    //! implementation returns ExsciScintillaBase::SCLEX_CONTAINER.
    //!
    //! \sa lexer()
    ...
    //! language is block structured, or ExsciScintilla::AiMaintain if not.
    //!
    //! \sa setAutoIndentStyle(), ExsciScintilla::AiMaintain,
    //! ExsciScintilla::AiOpening, ExsciScintilla::AiClosing
    int autoIndentStyle();
    ...
    //! Returns the ExsciScintilla instance that the lexer is currently attached
    //! to or 0 if it is unattached.
    ExsciScintilla *editor() const {return attached_editor;}

    //! The current set of APIs is set to \a apis.  If \a apis is 0 then any
    //! existing APIs for this lexer are removed.
    //!
    //! \sa apis()
    void setAPIs(ExsciAbstractAPIs *apis);
    ...
    //! \internal Set the ExsciScintilla instance that the lexer is attached to.
    ...
    //!
    //! \sa writeSettings(), ExsciScintilla::setLexer()
...
public slots:
    ...
    //!
    //! \sa autoIndentStyle(), ExsciScintilla::AiMaintain,
    //! ExsciScintilla::AiOpening, ExsciScintilla::AiClosing
...
private:
    ...
    QColor defColor;
    QColor defPaper;
    ExsciAbstractAPIs *apiSet;
    ExsciScintilla *attached_editor;

    void setStyleDefaults() const;
    StyleData &styleData(int style) const;

    ExsciLexer(const ExsciLexer &);
    ExsciLexer &operator=(const ExsciLexer &);
};
