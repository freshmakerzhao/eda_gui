...
#ifndef EXSCISCINTILLABASE_H
#define EXSCISCINTILLABASE_H
...
//! \brief The ExsciScintillaBase class implements the Scintilla editor widget
...
//! editor.
//!
//! ExsciScintillaBase is a port to Qt of Scintilla. It implements the standard
//! Scintilla API which consists of a number of messages each taking up to
//! two arguments.
//!
//! See ExsciScintilla for the implementation of a higher level API that is more
//! consistent with the rest of the Qt toolkit.
class EXSCINTILLA_EXPORT ExsciScintillaBase : public QAbstractScrollArea
{
    Q_OBJECT
...
public:
    ...
    enum
    {
        ...
        //! SC_CURSORREVERSEARROW.  Note that, currently, EXScintilla implements
    ...
    //! Construct an empty ExsciScintillaBase with parent \a parent.
    explicit ExsciScintillaBase(QWidget *parent = 0);

    //! Destroys the ExsciScintillaBase instance.
    virtual ~ExsciScintillaBase();

    //! Returns a pointer to a ExsciScintillaBase instance, or 0 if there isn't
    //! one.  This can be used by the higher level API to send messages that
    //! aren't associated with a particular instance.
    static ExsciScintillaBase *pool();
...
private:
    // This is needed to allow ExsciScintillaQt to emit this class's signals.
    friend class ExsciScintillaQt;
    ...
    ExsciScintillaBase(const ExsciScintillaBase &);
    ExsciScintillaBase &operator=(const ExsciScintillaBase &);
};
