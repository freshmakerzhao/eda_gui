// This module defines interface to the ExsciAPIs class.
...
class ExsciAPIsPrepared;
class ExsciAPIsWorker;
class ExsciLexer;


//! \brief The ExsciAPIs class provies an implementation of the textual API
...
//!
//! EXScintilla only deals with prepared API information and not the raw
...
//!
//! EXScintilla based applications that want to support large APIs would
...
class EXSCINTILLA_EXPORT ExsciAPIs : public ExsciAbstractAPIs
{
    Q_OBJECT

public:
    //! Constructs a ExsciAPIs instance attached to lexer \a lexer.  \a lexer
    //! becomes the instance's parent object although the instance can also be
    //! subsequently attached to other lexers.
    ExsciAPIs(ExsciLexer *lexer);

    //! Destroy the ExsciAPIs instance.
    virtual ~ExsciAPIs();
    ...
    //! by the EXSCIDIR environment variable.  If the environment variable isn't
    //! set then $HOME/.exsci is used.
    ...
    //! the associated lexer and saved in the directory defined by the EXSCIDIR
    //! environment variable.  If the environment variable isn't set then
    //! $HOME/.exsci is used.  Returns true if successful, otherwise false.
...
private:
    friend class ExsciAPIsPrepared;
    friend class ExsciAPIsWorker;
    ...
    ExsciAPIs(const ExsciAPIs &);
    ExsciAPIs &operator=(const ExsciAPIs &);
};
