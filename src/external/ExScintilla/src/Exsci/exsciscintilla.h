...
#ifndef EXSCISCINTILLA_H
#define EXSCISCINTILLA_H
...
class ExsciCommandSet;
class ExsciLexer;
class ExsciStyle;
class ExsciStyledText;
class ExsciListBoxQt;


//! \brief The ExsciScintilla class implements a higher level, more Qt-like,
//! API to the Scintilla editor widget.
//!
//! ExsciScintilla implements methods, signals and slots similar to those found
...
//! auto-indenting and auto-completion than that provided by ExsciScintillaBase.
class EXSCINTILLA_EXPORT ExsciScintilla : public ExsciScintillaBase
{
    Q_OBJECT
...
public:
    ...
    //! Construct an empty ExsciScintilla with parent \a parent.
    ExsciScintilla(QWidget *parent = 0);

    //! Destroys the ExsciScintilla instance.
    virtual ~ExsciScintilla();
    ...
    //! style.
    void annotate(int line, const QString &text, const ExsciStyle &style);

    //! Annotate the line \a line with the styled text \a text.
    void annotate(int line, const ExsciStyledText &text);

    //! Annotate the line \a line with the list of styled text \a text.
    void annotate(int line, const QList<ExsciStyledText> &text);
    ...
    //! is typically used by ExsciLexerCustom::styleText().
    //!
    //! \sa text()
    ...
    //!
    //! \sa setDocument()
    ExsciDocument document() const {return doc;}
    ...
    //! EXScintilla uses the combination of a line number and a character index
    ...
    //!
    //! \sa setMarginMask(), ExsciMarker, SCI_GETMARGINMASKN
    ...
    //!
    //! \sa clearRegisteredImages(), ExsciLexer::apiLoad()
    ...
    //!
    //! \sa document()
    void setDocument(const ExsciDocument &document);
    ...
    //! Set the margin text of line \a line with the text \a text using the
    //! style \a style.
    void setMarginText(int line, const QString &text, const ExsciStyle &style);

    //! Set the margin text of line \a line with the styled text \a text.
    void setMarginText(int line, const ExsciStyledText &text);

    //! Set the margin text of line \a line with the list of styled text \a
    //! text.
    void setMarginText(int line, const QList<ExsciStyledText> &text);
    ...
    //! For performance, EXScintilla does not measure the display width of the
    ...
    //! pixels assumed by EXScintilla to \a pixelWidth.  The default value is
    //! 2000.
    //!
    //! \sa scrollWidth(), setScrollWidthTracking()
    ...
    ExsciCommandSet *standardCommands() const {return stdCmds;}
    ...
    //! typically used by ExsciLexerCustom::styleText().
    //!
    //! \sa bytes(), setText()
...
public slots:
    ...
    //!
    //! \sa lexer()
    virtual void setLexer(ExsciLexer *lexer = 0);
    ...
    //!
    //! \sa marginMarkerMask(), ExsciMarker, SCI_SETMARGINMASKN
...
private:
    ...
    ScintillaBytes styleText(const QList<ExsciStyledText> &styled_text,
    ...
    QPointer<ExsciLexer> lex;
    ExsciCommandSet *stdCmds;
    ExsciDocument doc;
    ...
    // The following allow ExsciListBoxQt to distinguish between an
    // auto-completion list and a user list, and to return the full selection
    // of an auto-completion list.
    friend class ExsciListBoxQt;

    QString acSelection;
    bool isAutoCompletionList() const;

    void set_shortcut(QAction *action, ExsciCommand::Command cmd_id) const;

    ExsciScintilla(const ExsciScintilla &);
    ExsciScintilla &operator=(const ExsciScintilla &);
};
