// This module defines interface to the ExsciPrinter class.



#ifndef EXSCIPRINTER_H
#define EXSCIPRINTER_H
...
//! \brief The ExsciPrinter class is a sub-class of the Qt QPrinter class that
...
class EXSCINTILLA_EXPORT ExsciPrinter : public QPrinter
{
public:
    //! Constructs a printer paint device with mode \a mode.
    ExsciPrinter(PrinterMode mode = ScreenResolution);

    //! Destroys the ExsciPrinter instance.
    virtual ~ExsciPrinter();
    ...
    //! error.
    virtual int printRange(ExsciScintillaBase *qsb, int from = -1, int to = -1);

    //! Return the line wrap mode used when printing.  The default is
    //! ExsciScintilla::WrapWord.
    //!
    //! \sa setWrapMode()
    ExsciScintilla::WrapMode wrapMode() const {return wrap;}

    //! Sets the line wrap mode used when printing to \a wmode.
    //!
    //! \sa wrapMode()
    virtual void setWrapMode(ExsciScintilla::WrapMode wmode);

private:
    int mag;
    ExsciScintilla::WrapMode wrap;

    ExsciPrinter(const ExsciPrinter &);
    ExsciPrinter &operator=(const ExsciPrinter &);
};
