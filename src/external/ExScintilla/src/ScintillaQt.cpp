...
// The ctor.
ExsciScintillaQt::ExsciScintillaQt(ExsciScintillaBase *qsb_)
...
// The dtor.
ExsciScintillaQt::~ExsciScintillaQt()
{ 
    Finalise();
}
...
void ExsciScintillaQt::Initialise()
{
}


// Tidy up the instance.
void ExsciScintillaQt::Finalise()
{
    for (int i = 0; i <= static_cast<int>(tickPlatform); ++i)
...
// Start a drag.
void ExsciScintillaQt::StartDrag()
{
    ...
    SetDragPosition(EXSCI_SCI_NAMESPACE(SelectionPosition)());
    inDragDrop = ddNone;
}


// Re-implement to trap certain messages.
sptr_t ExsciScintillaQt::WndProc(unsigned int iMessage, uptr_t wParam,
        sptr_t lParam)
{
    switch (iMessage)
    {
...
// Windows nonsense.
sptr_t ExsciScintillaQt::DefWndProc(unsigned int, uptr_t, sptr_t)
{
    return 0;
}
...
void ExsciScintillaQt::SetMouseCapture(bool on)
{
...
bool ExsciScintillaQt::HaveMouseCapture()
{
...
void ExsciScintillaQt::SetVerticalScrollPos()
{
...
void ExsciScintillaQt::SetHorizontalScrollPos()
{
...
bool ExsciScintillaQt::ModifyScrollBars(int nMax,int nPage)
{
    bool modified = false;
    QScrollBar *sb;
...
void ExsciScintillaQt::ReconfigureScrollBars()
{
...
void ExsciScintillaQt::NotifyChange()
{
...
void ExsciScintillaQt::NotifyParent(SCNotification scn)
{
...
QMimeData *ExsciScintillaQt::mimeSelection(
...
void ExsciScintillaQt::CopyToClipboard(
...
// Implement copy.
void ExsciScintillaQt::Copy()
{
    if (!sel.Empty())
    {
        EXSCI_SCI_NAMESPACE(SelectionText) text;
...
void ExsciScintillaQt::pasteFromClipboard(QClipboard::Mode mode)
{
    int len;
    const char *s;
    bool rectangular;
...
void ExsciScintillaQt::CreateCallTipWindow(EXSCI_SCI_NAMESPACE(PRectangle) rc)
{
    if (!ct.wCallTip.Created())
        ct.wCallTip = ct.wDraw = new ExsciSciCallTip(qsb, this);

    ExsciSciCallTip *w = reinterpret_cast<ExsciSciCallTip *>(ct.wCallTip.GetID());
...
void ExsciScintillaQt::AddToPopUp(const char *label, int cmd, bool enabled)
{
    ExsciSciPopup *pm = static_cast<ExsciSciPopup *>(popup.GetID());
...
// Claim the selection.
void ExsciScintillaQt::ClaimSelection()
{
...
void ExsciScintillaQt::UnclaimSelection()
{
    if (primarySelection)
    {
        primarySelection = false;
...
sptr_t ExsciScintillaQt::DirectFunction(ExsciScintillaQt *sciThis, unsigned int iMessage,
...
void ExsciScintillaQt::paintEvent(QPaintEvent *e)
{
    EXSCI_SCI_NAMESPACE(Surface) *sw;
    ...
    EXSCI_SCI_NAMESPACE(PRectangle) rcClient = GetClientRectangle();
    paintingAllText = rcPaint.Contains(rcClient);

    sw = EXSCI_SCI_NAMESPACE(Surface)::Allocate(SC_TECHNOLOGY_DEFAULT);
    if (!sw)
        return;
...
void ExsciScintillaQt::timerEvent(QTimerEvent *e)
{
    for (int i = 0; i <= static_cast<int>(tickPlatform); ++i)
...
bool ExsciScintillaQt::FineTickerAvailable()
{
    return true;
}
...
void ExsciScintillaQt::FineTickerCancel(TickReason reason)
{
...
bool ExsciScintillaQt::FineTickerRunning(TickReason reason)
{
...
void ExsciScintillaQt::FineTickerStart(TickReason reason, int ms, int)
{
    int &ticker = timers[static_cast<int>(reason)];
...
bool ExsciScintillaQt::SetIdle(bool on)
{
    if (on)
    {
        if (!idler.state)
