// This module implements the ExsciAPIs class.
...
struct ExsciAPIsPrepared
{
    ...
    QMap<QString, ExsciAPIs::WordIndexList> wdict;
...
QStringList ExsciAPIsPrepared::apiWords(int api_idx, const QStringList &wseps,
        bool strip_image) const
{
...
QString ExsciAPIsPrepared::apiBaseName(const QString &api)
{
    QString base = api;
    int tail = base.indexOf('(');
...
class ExsciAPIsWorker : public QThread
{
public:
    ExsciAPIsWorker(ExsciAPIs *apis);
    virtual ~ExsciAPIsWorker();

    virtual void run();

    ExsciAPIsPrepared *prepared;
...
ExsciAPIsWorker::ExsciAPIsWorker(ExsciAPIs *apis)
...
ExsciAPIsWorker::~ExsciAPIsWorker()
{
...
void ExsciAPIsWorker::run()
{
    ...
    for (int a = 0; a < prepared->raw_apis.count(); ++a)
    {
        ...
        for (int w = 0; w < words.count(); ++w)
        {
            ...
            ExsciAPIs::WordIndexList wil = prepared->wdict[word];
            ...
            wil.append(ExsciAPIs::WordIndex(a, w));
...
// The ctor.
ExsciAPIs::ExsciAPIs(ExsciLexer *lexer)
    : ExsciAbstractAPIs(lexer), worker(0), origin_len(0)
{
    prep = new ExsciAPIsPrepared;
}
...
bool ExsciAPIs::load(const QString &filename)
{
    QFile f(filename);
...
void ExsciAPIs::add(const QString &entry)
{
    apis.append(entry);
}
...
void ExsciAPIs::remove(const QString &entry)
{
...
// supplied context.
QStringList ExsciAPIs::positionOrigin(const QStringList &context, QString &path)
{
...
bool ExsciAPIs::originStartsWith(const QString &path, const QString &wsep)
{
...
void ExsciAPIs::updateAutoCompletionList(const QStringList &context,
        QStringList &list)
{
    QString path;
    ...
    if (origin_len > 0)
    {
        ...
        {
            QString base = ExsciAPIsPrepared::apiBaseName(*it);
...
const ExsciAPIs::WordIndexList *ExsciAPIs::wordIndexOf(const QString &word) const
{
    QString csword;
...
void ExsciAPIs::lastCompleteWord(const QString &word, QStringList &with_context, bool &unambig)
{
    // Get the possible API entries if any.
...
void ExsciAPIs::lastPartialWord(const QString &word, QStringList &with_context, bool &unambig)
{
    if (lexer()->caseSensitive())
    {
...
void ExsciAPIs::autoCompletionSelected(const QString &selection)
{
...
void ExsciAPIs::addAPIEntries(const WordIndexList &wl, bool complete,
...
QStringList ExsciAPIs::callTips(const QStringList &context, int commas,
        ExsciScintilla::CallTipsStyle style, QList<int> &shifts)
{
    QString path;
    QStringList new_context = positionOrigin(context, path);
    QStringList wseps = lexer()->autoCompletionWordSeparators();
    ...
    if (origin_len > 0)
    {
        ...
        if (style == ExsciScintilla::CallTipsContext)
        {
            shift = ctstart;
            ctstart = 0;
        }
    ...
    else
    {
        ...
        if (wil)
            for (int i = 0; i < wil->count(); ++i)
            {
                ...
                if (style == ExsciScintilla::CallTipsNoContext)
                {
                    shifts << 0;
...
bool ExsciAPIs::enoughCommas(const QString &s, int commas)
{
    int end = s.indexOf(')');
...
void ExsciAPIs::prepare()
{
    // Handle the trivial case.
    if (worker)
        return;

    ExsciAPIsPrepared *new_apis = new ExsciAPIsPrepared;
    new_apis->raw_apis = apis;

    worker = new ExsciAPIsWorker(this);
...
void ExsciAPIs::cancelPreparation()
{
    deleteWorker();
}
...
bool ExsciAPIs::isPrepared(const QString &filename) const
{
...
bool ExsciAPIs::loadPrepared(const QString &filename)
{
    QString pname = prepName(filename);
...
bool ExsciAPIs::savePrepared(const QString &filename) const
{
...
QString ExsciAPIs::defaultPreparedName() const
{
...
QString ExsciAPIs::prepName(const QString &filename, bool mkpath) const
{
    ...
    QString pdname;
    char *exsci = getenv("EXSCIDIR");

    if (exsci)
        pdname = exsci;
    else
    {
        static const char *exsci_dir = ".exsci";

        QDir pd = QDir::home();

        if (mkpath && !pd.exists(exsci_dir) && !pd.mkdir(exsci_dir))
            return QString();

        pdname = pd.filePath(exsci_dir);
    }
...
QStringList ExsciAPIs::installedAPIFiles() const
{
    QString qtdir = QLibraryInfo::location(QLibraryInfo::DataPath);

    QDir apidir = QDir(QString("%1/exsci/api/%2").arg(qtdir).arg(lexer()->lexer()));
