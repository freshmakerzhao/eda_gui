...
// The ctor.
ExsciScintilla::ExsciScintilla(QWidget *parent)
    : ExsciScintillaBase(parent),
      ...
      use_single(AcusNever), explicit_fillups(""), fillups_enabled(false)
{
    ...
    stdCmds = new ExsciCommandSet(this);
...
QColor ExsciScintilla::color() const
{
...
// Set the text colour.
void ExsciScintilla::setColor(const QColor &c)
{
    if (lex.isNull())
    {
...
bool ExsciScintilla::overwriteMode() const
{
...
void ExsciScintilla::setOverwriteMode(bool overwrite)
{
...
QColor ExsciScintilla::paper() const
{
...
void ExsciScintilla::setPaper(const QColor &c)
{
    if (lex.isNull())
    {
        // Assume style 0 applies to everything so that we don't need to use
...
// Set the default font.
void ExsciScintilla::setFont(const QFont &f)
{
    if (lex.isNull())
    {
        // Assume style 0 applies to everything so that we don't need to use
...
void ExsciScintilla::setAutoIndent(bool autoindent)
{
    autoInd = autoindent;
}
...
void ExsciScintilla::setBraceMatching(BraceMatch bm)
{
    braceMode = bm;
}
...
void ExsciScintilla::handleCharAdded(int ch)
{
...
bool ExsciScintilla::isCallTipActive() const
{
...
void ExsciScintilla::callTip()
{
    ExsciAbstractAPIs *apis = lex->apis();
...
void ExsciScintilla::handleCallTipClick(int dir)
{
    int nr_entries = ct_entries.count();
...
int ExsciScintilla::adjustedCallTipPosition(int ctshift) const
{
    int ct = ctPos;
...
// of the context.
QStringList ExsciScintilla::apiContext(int pos, int &context_start,
        int &last_word_start)
{
    enum {
        Either,
...
bool ExsciScintilla::getSeparator(int &pos) const
{
    int opos = pos;
...
QString ExsciScintilla::getWord(int &pos) const
{
    QString word;
    bool numeric = true;
    char ch;
...
char ExsciScintilla::getCharacter(int &pos) const
{
    if (pos <= 0)
        return '\0';
...
bool ExsciScintilla::isStartChar(char ch) const
{
    QString s = QChar(ch);
...
void ExsciScintilla::startAutoCompletion(AutoCompletionSource acs,
...
void ExsciScintilla::maintainIndentation(char ch, long pos)
{
    if (ch != '\r' && ch != '\n')
        return;
...
void ExsciScintilla::autoIndentation(char ch, long pos)
{
    int curr_line = SendScintilla(SCI_LINEFROMPOSITION, pos);
...
void ExsciScintilla::autoIndentLine(long pos, int line, int indent)
{
    if (indent < 0)
        return;
...
// significant before).
int ExsciScintilla::blockIndent(int line)
{
    if (line < 0)
        return 0;
...
bool ExsciScintilla::rangeIsWhitespace(long spos, long epos)
{
    while (spos < epos)
    {
...
ExsciScintilla::IndentState ExsciScintilla::getIndentState(int line)
{
    IndentState istate;
...
int ExsciScintilla::findStyledWord(const char *text, int style,
        const char *words)
{
    if (!words)
        return -1;
...
bool ExsciScintilla::isUtf8() const
{
...
// Set the code page.
void ExsciScintilla::setUtf8(bool cp)
{
...
ExsciScintilla::EolMode ExsciScintilla::eolMode() const
{
...
void ExsciScintilla::setEolMode(EolMode mode)
{
...
void ExsciScintilla::convertEols(EolMode mode)
{
...
// Add an edge column.
void ExsciScintilla::addEdgeColumn(int colnr, const QColor &col)
{
...
void ExsciScintilla::clearEdgeColumns()
{
...
QColor ExsciScintilla::edgeColor() const
{
...
// Set the edge colour.
void ExsciScintilla::setEdgeColor(const QColor &col)
{
...
int ExsciScintilla::edgeColumn() const
{
...
// Set the edge column.
void ExsciScintilla::setEdgeColumn(int colnr)
{
...
ExsciScintilla::EdgeMode ExsciScintilla::edgeMode() const
{
...
// Set the edge mode.
void ExsciScintilla::setEdgeMode(EdgeMode mode)
{
...
bool ExsciScintilla::eolVisibility() const
{
...
void ExsciScintilla::setEolVisibility(bool visible)
{
...
int ExsciScintilla::extraAscent() const
{
...
void ExsciScintilla::setExtraAscent(int extra)
{
...
int ExsciScintilla::extraDescent() const
{
...
void ExsciScintilla::setExtraDescent(int extra)
{
...
int ExsciScintilla::whitespaceSize() const
{
...
void ExsciScintilla::setWhitespaceSize(int size)
{
...
void ExsciScintilla::setWhitespaceBackgroundColor(const QColor &col)
{
...
void ExsciScintilla::setWhitespaceForegroundColor(const QColor &col)
{
...
ExsciScintilla::WhitespaceVisibility ExsciScintilla::whitespaceVisibility() const
{
...
void ExsciScintilla::setWhitespaceVisibility(WhitespaceVisibility mode)
{
...
ExsciScintilla::TabDrawMode ExsciScintilla::tabDrawMode() const
{
...
void ExsciScintilla::setTabDrawMode(TabDrawMode mode)
{
...
ExsciScintilla::WrapMode ExsciScintilla::wrapMode() const
{
...
void ExsciScintilla::setWrapMode(WrapMode mode)
{
...
ExsciScintilla::WrapIndentMode ExsciScintilla::wrapIndentMode() const
{
...
void ExsciScintilla::setWrapIndentMode(WrapIndentMode mode)
{
...
void ExsciScintilla::setWrapVisualFlags(WrapVisualFlag endFlag,
...
void ExsciScintilla::setFolding(FoldStyle folding, int margin)
{
    fold = folding;
    foldmargin = margin;
...
void ExsciScintilla::clearFolds()
{
    recolor();
...
void ExsciScintilla::setFoldMarker(int marknr, int mark)
{
...
void ExsciScintilla::foldClick(int lineClick, int bstate)
{
...
// SciTE.
void ExsciScintilla::foldExpand(int &line, bool doExpand, bool force,
...
void ExsciScintilla::foldAll(bool children)
{
    recolor();
...
void ExsciScintilla::foldChanged(int line,int levelNow,int levelPrev)
{
...
void ExsciScintilla::foldLine(int line)
{
...
QList<int> ExsciScintilla::contractedFolds() const
{
    QList<int> folds;
...
void ExsciScintilla::setContractedFolds(const QList<int> &folds)
{
...
void ExsciScintilla::handleModified(int pos, int mtype, const char *text,
...
void ExsciScintilla::zoomIn(int range)
{
...
void ExsciScintilla::zoomOut(int range)
{
    zoomTo(SendScintilla(SCI_GETZOOM) - range);
}
...
void ExsciScintilla::zoomTo(int size)
{
    if (size < -10)
        size = -10;
    else if (size > 20)
...
bool ExsciScintilla::findFirst(const QString &expr, bool re, bool cs, bool wo,
...
bool ExsciScintilla::findFirstInSelection(const QString &expr, bool re, bool cs,
...
void ExsciScintilla::replace(const QString &replaceStr)
{
    if (findState.status == FindState::Idle)
        return;
...
bool ExsciScintilla::isModified() const
{
...
void ExsciScintilla::setModified(bool m)
{
    if (!m)
...
void ExsciScintilla::handleIndicatorClick(int pos, int modifiers)
{
...
void ExsciScintilla::handleIndicatorRelease(int pos, int modifiers)
{
    int state = mapModifiers(modifiers);
    int line, index;
...
void ExsciScintilla::handleMarginClick(int pos, int modifiers, int margin)
{
    int state = mapModifiers(modifiers);
...
void ExsciScintilla::handleMarginRightClick(int pos, int modifiers, int margin)
{
    int state = mapModifiers(modifiers);
    int line = SendScintilla(SCI_LINEFROMPOSITION, pos);
...
void ExsciScintilla::handleSavePointReached()
{
...
void ExsciScintilla::handleSavePointLeft()
{
...
void ExsciScintilla::handleSelectionChanged(bool yes)
{
    selText = yes;
...
void ExsciScintilla::getSelection(int *lineFrom, int *indexFrom, int *lineTo,
        int *indexTo) const
{
    if (selText)
    {
...
void ExsciScintilla::setSelection(int lineFrom, int indexFrom, int lineTo,
        int indexTo)
{
...
void ExsciScintilla::setSelectionBackgroundColor(const QColor &col)
{
    int alpha = col.alpha();
...
void ExsciScintilla::setSelectionForegroundColor(const QColor &col)
{
...
void ExsciScintilla::resetSelectionBackgroundColor()
{
...
void ExsciScintilla::resetSelectionForegroundColor()
{
...
void ExsciScintilla::setSelectionToEol(bool filled)
{
...
bool ExsciScintilla::selectionToEol() const
{
...
void ExsciScintilla::setCaretWidth(int width)
{
...
void ExsciScintilla::setCaretForegroundColor(const QColor &col)
{
...
void ExsciScintilla::setCaretLineBackgroundColor(const QColor &col)
{
    int alpha = col.alpha();
...
void ExsciScintilla::setCaretLineVisible(bool enable)
{
...
void ExsciScintilla::setHotspotBackgroundColor(const QColor &col)
{
    SendScintilla(SCI_SETSELBACK, 1, col);
}
...
void ExsciScintilla::setHotspotForegroundColor(const QColor &col)
{
...
void ExsciScintilla::resetHotspotBackgroundColor()
{
    SendScintilla(SCI_SETSELBACK, 0UL);
}
...
void ExsciScintilla::resetHotspotForegroundColor()
{
...
void ExsciScintilla::setHotspotUnderline(bool enable)
{
...
void ExsciScintilla::setHotspotWrap(bool enable)
{
...
bool ExsciScintilla::isReadOnly() const
{
...
void ExsciScintilla::setReadOnly(bool ro)
{
...
void ExsciScintilla::append(const QString &text)
{
    bool ro = ensureRW();
...
void ExsciScintilla::insert(const QString &text)
{
    insertAtPos(text, -1);
}
...
void ExsciScintilla::insertAt(const QString &text, int line, int index)
{
...
void ExsciScintilla::insertAtPos(const QString &text, int pos)
{
    bool ro = ensureRW();
...
void ExsciScintilla::beginUndoAction()
{
    SendScintilla(SCI_BEGINUNDOACTION);
}
...
void ExsciScintilla::endUndoAction()
{
    SendScintilla(SCI_ENDUNDOACTION);
}
...
bool ExsciScintilla::isRedoAvailable() const
{
...
bool ExsciScintilla::isUndoAvailable() const
{
...
int ExsciScintilla::lines() const
{
...
int ExsciScintilla::lineAt(const QPoint &pos) const
{
...
int ExsciScintilla::lineLength(int line) const
{
...
int ExsciScintilla::length() const
{
...
void ExsciScintilla::removeSelectedText()
{
...
void ExsciScintilla::replaceSelectedText(const QString &text)
{
...
QString ExsciScintilla::selectedText() const
{
    if (!selText)
        return QString();
...
QString ExsciScintilla::text() const
{
    int buflen = length() + 1;
...
QString ExsciScintilla::text(int line) const
{
...
QString ExsciScintilla::text(int start, int end) const
{
...
QByteArray ExsciScintilla::bytes(int start, int end) const
{
...
// Set the given text.
void ExsciScintilla::setText(const QString &text)
{
    bool ro = ensureRW();
...
void ExsciScintilla::getCursorPosition(int *line, int *index) const
{
...
void ExsciScintilla::setCursorPosition(int line, int index)
{
...
void ExsciScintilla::ensureCursorVisible()
{
...
void ExsciScintilla::ensureLineVisible(int line)
{
...
void ExsciScintilla::selectAll(bool select)
{
    if (select)
...
int ExsciScintilla::indentation(int line) const
{
...
void ExsciScintilla::setIndentation(int line, int indentation)
{
    SendScintilla(SCI_BEGINUNDOACTION);
...
// Indent a line.
void ExsciScintilla::indent(int line)
{
...
// Unindent a line.
void ExsciScintilla::unindent(int line)
{
...
int ExsciScintilla::currentIndent() const
{
...
int ExsciScintilla::indentWidth() const
{
...
bool ExsciScintilla::indentationGuides() const
{
...
void ExsciScintilla::setIndentationGuides(bool enable)
{
    int iv;
...
void ExsciScintilla::setIndentationGuidesBackgroundColor(const QColor &col)
{
...
void ExsciScintilla::setIndentationGuidesForegroundColor(const QColor &col)
{
...
int ExsciScintilla::indentationWidth() const
{
...
void ExsciScintilla::setIndentationWidth(int width)
{
...
int ExsciScintilla::tabWidth() const
{
...
// Set the tab width.
void ExsciScintilla::setTabWidth(int width)
{
...
bool ExsciScintilla::backspaceUnindents() const
{
...
void ExsciScintilla::setBackspaceUnindents(bool unindents)
{
...
bool ExsciScintilla::tabIndents() const
{
...
void ExsciScintilla::setTabIndents(bool indents)
{
...
bool ExsciScintilla::indentationsUseTabs() const
{
...
void ExsciScintilla::setIndentationsUseTabs(bool tabs)
{
...
int ExsciScintilla::margins() const
{
...
void ExsciScintilla::setMargins(int margins)
{
...
QColor ExsciScintilla::marginBackgroundColor(int margin) const
{
...
void ExsciScintilla::setMarginBackgroundColor(int margin, const QColor &col)
{
...
int ExsciScintilla::marginOptions() const
{
...
void ExsciScintilla::setMarginOptions(int options)
{
...
ExsciScintilla::MarginType ExsciScintilla::marginType(int margin) const
{
...
// Set the margin type.
void ExsciScintilla::setMarginType(int margin, ExsciScintilla::MarginType type)
{
...
// Clear margin text.
void ExsciScintilla::clearMarginText(int line)
{
    if (line < 0)
...
// Annotate a line.
void ExsciScintilla::setMarginText(int line, const QString &text, int style)
{
...
// Annotate a line.
void ExsciScintilla::setMarginText(int line, const QString &text, const ExsciStyle &style)
{
    style.apply(this);
...
// Annotate a line.
void ExsciScintilla::setMarginText(int line, const ExsciStyledText &text)
{
    text.apply(this);
...
// Annotate a line.
void ExsciScintilla::setMarginText(int line, const QList<ExsciStyledText> &text)
{
    char *styles;
...
bool ExsciScintilla::marginLineNumbers(int margin) const
{
...
void ExsciScintilla::setMarginLineNumbers(int margin, bool lnrs)
{
...
int ExsciScintilla::marginMarkerMask(int margin) const
{
...
void ExsciScintilla::setMarginMarkerMask(int margin,int mask)
{
...
bool ExsciScintilla::marginSensitivity(int margin) const
{
...
void ExsciScintilla::setMarginSensitivity(int margin,bool sens)
{
...
int ExsciScintilla::marginWidth(int margin) const
{
...
void ExsciScintilla::setMarginWidth(int margin, int width)
{
...
void ExsciScintilla::setMarginWidth(int margin, const QString &s)
{
...
void ExsciScintilla::setMarginsBackgroundColor(const QColor &col)
{
...
void ExsciScintilla::setMarginsForegroundColor(const QColor &col)
{
...
void ExsciScintilla::setMarginsFont(const QFont &f)
{
...
// Define an indicator.
int ExsciScintilla::indicatorDefine(IndicatorStyle style, int indicatorNumber)
{
...
bool ExsciScintilla::indicatorDrawUnder(int indicatorNumber) const
{
...
void ExsciScintilla::setIndicatorDrawUnder(bool under, int indicatorNumber)
{
...
void ExsciScintilla::setIndicatorForegroundColor(const QColor &col,
        int indicatorNumber)
{
    if (indicatorNumber < INDIC_IME)
    {
...
void ExsciScintilla::setIndicatorHoverForegroundColor(const QColor &col,
        int indicatorNumber)
{
    if (indicatorNumber < INDIC_IME)
    {
...
void ExsciScintilla::setIndicatorHoverStyle(IndicatorStyle style,
        int indicatorNumber)
{
    if (indicatorNumber < INDIC_IME)
    {
...
void ExsciScintilla::setIndicatorOutlineColor(const QColor &col, int indicatorNumber)
{
    if (indicatorNumber < INDIC_IME)
    {
        int alpha = col.alpha();
...
void ExsciScintilla::fillIndicatorRange(int lineFrom, int indexFrom,
...
void ExsciScintilla::clearIndicatorRange(int lineFrom, int indexFrom,
        int lineTo, int indexTo, int indicatorNumber)
{
    if (indicatorNumber < INDIC_IME)
    {
...
int ExsciScintilla::markerDefine(MarkerSymbol sym, int markerNumber)
{
...
int ExsciScintilla::markerDefine(char ch, int markerNumber)
{
    checkMarker(markerNumber);
...
int ExsciScintilla::markerDefine(const QPixmap &pm, int markerNumber)
{
    checkMarker(markerNumber);
...
int ExsciScintilla::markerDefine(const QImage &im, int markerNumber)
{
    checkMarker(markerNumber);
...
int ExsciScintilla::markerAdd(int linenr, int markerNumber)
{
...
unsigned ExsciScintilla::markersAtLine(int linenr) const
{
...
void ExsciScintilla::markerDelete(int linenr, int markerNumber)
{
...
void ExsciScintilla::markerDeleteAll(int markerNumber)
{
    if (markerNumber <= MARKER_MAX)
    {
        if (markerNumber < 0)
...
void ExsciScintilla::markerDeleteHandle(int mhandle)
{
...
int ExsciScintilla::markerLine(int mhandle) const
{
...
int ExsciScintilla::markerFindNext(int linenr, unsigned mask) const
{
...
int ExsciScintilla::markerFindPrevious(int linenr, unsigned mask) const
{
...
void ExsciScintilla::setMarkerBackgroundColor(const QColor &col, int markerNumber)
{
    if (markerNumber <= MARKER_MAX)
    {
        int alpha = col.alpha();
...
void ExsciScintilla::setMarkerForegroundColor(const QColor &col, int markerNumber)
{
    if (markerNumber <= MARKER_MAX)
    {
        if (markerNumber < 0)
...
void ExsciScintilla::checkMarker(int &markerNumber)
{
...
void ExsciScintilla::checkIndicator(int &indicatorNumber)
{
...
void ExsciScintilla::allocateId(int &id, unsigned &allocated, int min, int max)
{
    if (id >= 0)
    {
...
void ExsciScintilla::resetFoldMarginColors()
{
...
void ExsciScintilla::setFoldMarginColors(const QColor &fore, const QColor &back)
{
...
void ExsciScintilla::setCallTipsBackgroundColor(const QColor &col)
{
...
void ExsciScintilla::setCallTipsForegroundColor(const QColor &col)
{
...
void ExsciScintilla::setCallTipsHighlightColor(const QColor &col)
{
...
void ExsciScintilla::setMatchedBraceBackgroundColor(const QColor &col)
{
...
void ExsciScintilla::setMatchedBraceForegroundColor(const QColor &col)
{
...
void ExsciScintilla::setMatchedBraceIndicator(int indicatorNumber)
{
...
void ExsciScintilla::resetMatchedBraceIndicator()
{
...
void ExsciScintilla::setUnmatchedBraceBackgroundColor(const QColor &col)
{
...
void ExsciScintilla::setUnmatchedBraceForegroundColor(const QColor &col)
{
...
void ExsciScintilla::setUnmatchedBraceIndicator(int indicatorNumber)
{
...
void ExsciScintilla::resetUnmatchedBraceIndicator()
{
...
// Detach any lexer.
void ExsciScintilla::detachLexer()
{
    if (!lex.isNull())
    {
        lex->setEditor(0);
...
// Set the lexer.
void ExsciScintilla::setLexer(ExsciLexer *lexer)
{
    // Detach any current lexer.
    detachLexer();
...
void ExsciScintilla::setLexerStyle(int style)
{
...
ExsciLexer *ExsciScintilla::lexer() const
{
    return lex;
}
...
void ExsciScintilla::handleStyleColorChange(const QColor &c, int style)
{
...
void ExsciScintilla::handleStyleEolFillChange(bool eolfill, int style)
{
...
void ExsciScintilla::handleStyleFontChange(const QFont &f, int style)
{
...
void ExsciScintilla::setStylesFont(const QFont &f, int style)
{
...
void ExsciScintilla::handleStylePaperChange(const QColor &c, int style)
{
...
void ExsciScintilla::handlePropertyChange(const char *prop, const char *val)
{
...
void ExsciScintilla::handleUpdateUI(int)
{
...
void ExsciScintilla::braceMatch()
{
...
long ExsciScintilla::checkBrace(long pos, int brace_style, bool &colonMode)
{
    long brace_pos = -1;
    char ch = SendScintilla(SCI_GETCHARAT, pos);
...
// a pair of braces.
bool ExsciScintilla::findMatchingBrace(long &brace, long &other,BraceMatch mode)
{
...
void ExsciScintilla::moveToMatchingBrace()
{
...
void ExsciScintilla::selectToMatchingBrace()
{
...
void ExsciScintilla::gotoMatchingBrace(bool select)
{
    long braceAtCaret;
    long braceOpposite;
...
int ExsciScintilla::positionFromLineIndex(int line, int index) const
{
...
void ExsciScintilla::lineIndexFromPosition(int position, int *line, int *index) const
{
...
void ExsciScintilla::setAutoCompletionSource(AutoCompletionSource source)
{
    acSource = source;
}
...
void ExsciScintilla::setAutoCompletionThreshold(int thresh)
{
    acThresh = thresh;
}
...
void ExsciScintilla::setAutoCompletionWordSeparators(const QStringList &separators)
{
    if (lex.isNull())
        wseps = separators;
}
...
void ExsciScintilla::autoCompleteFromAll()
{
...
void ExsciScintilla::autoCompleteFromAPIs()
{
...
void ExsciScintilla::autoCompleteFromDocument()
{
...
bool ExsciScintilla::isWordCharacter(char ch) const
{
...
const char *ExsciScintilla::wordCharacters() const
{
    return wchars;
}
...
void ExsciScintilla::recolor(int start, int end)
{
...
void ExsciScintilla::registerImage(int id, const QPixmap &pm)
{
...
void ExsciScintilla::registerImage(int id, const QImage &im)
{
    SendScintilla(SCI_RGBAIMAGESETHEIGHT, im.height());
    SendScintilla(SCI_RGBAIMAGESETWIDTH, im.width());
...
void ExsciScintilla::clearRegisteredImages()
{
...
void ExsciScintilla::setAutoCompletionFillupsEnabled(bool enable)
{
    const char *fillups;
...
bool ExsciScintilla::autoCompletionFillupsEnabled() const
{
...
void ExsciScintilla::setAutoCompletionFillups(const char *fillups)
{
...
void ExsciScintilla::setAutoCompletionCaseSensitivity(bool cs)
{
...
bool ExsciScintilla::autoCompletionCaseSensitivity() const
{
...
void ExsciScintilla::setAutoCompletionReplaceWord(bool replace)
{
...
bool ExsciScintilla::autoCompletionReplaceWord() const
{
...
void ExsciScintilla::setAutoCompletionUseSingle(AutoCompletionUseSingle single)
{
    use_single = single;
}
...
ExsciScintilla::AutoCompletionUseSingle ExsciScintilla::autoCompletionUseSingle() const
{
    return use_single;
}
...
void ExsciScintilla::setAutoCompletionShowSingle(bool single)
{
...
bool ExsciScintilla::autoCompletionShowSingle() const
{
...
void ExsciScintilla::setCallTipsPosition(CallTipsPosition position)
{
...
void ExsciScintilla::setCallTipsStyle(CallTipsStyle style)
{
...
void ExsciScintilla::setCallTipsVisible(int nr)
{
    maxCallTips = nr;
}
...
void ExsciScintilla::setDocument(const ExsciDocument &document)
{
...
int ExsciScintilla::firstVisibleLine() const
{
...
void ExsciScintilla::setFirstVisibleLine(int linenr)
{
...
int ExsciScintilla::textHeight(int linenr) const
{
...
bool ExsciScintilla::isListActive() const
{
...
void ExsciScintilla::cancelList()
{
...
void ExsciScintilla::handleAutoCompletionSelection()
{
    if (!lex.isNull())
    {
        ExsciAbstractAPIs *apis = lex->apis();
...
// Display a user list.
void ExsciScintilla::showUserList(int id, const QStringList &list)
{
...
void ExsciScintilla::handleUserListSelection(const char *text, int id)
{
...
bool ExsciScintilla::caseSensitive() const
{
...
// user list.
bool ExsciScintilla::isAutoCompletionList() const
{
...
bool ExsciScintilla::read(QIODevice *io)
{
...
bool ExsciScintilla::write(QIODevice *io) const
{
...
QString ExsciScintilla::wordAtLineIndex(int line, int index) const
{
...
// Return the word at the given coordinates.
QString ExsciScintilla::wordAtPoint(const QPoint &point) const
{
...
QString ExsciScintilla::wordAtPosition(int position) const
{
    if (position < 0)
        return QString();
...
ExsciScintilla::AnnotationDisplay ExsciScintilla::annotationDisplay() const
{
...
void ExsciScintilla::setAnnotationDisplay(ExsciScintilla::AnnotationDisplay display)
{
...
// Clear annotations.
void ExsciScintilla::clearAnnotations(int line)
{
    if (line >= 0)
...
// Annotate a line.
void ExsciScintilla::annotate(int line, const QString &text, int style)
{
...
// Annotate a line.
void ExsciScintilla::annotate(int line, const QString &text, const ExsciStyle &style)
{
    style.apply(this);
...
// Annotate a line.
void ExsciScintilla::annotate(int line, const ExsciStyledText &text)
{
    text.apply(this);
...
// Annotate a line.
void ExsciScintilla::annotate(int line, const QList<ExsciStyledText> &text)
{
    char *styles;
    ScintillaBytes styled_text = styleText(text, &styles,
...
QString ExsciScintilla::annotation(int line) const
{
...
ExsciScintillaBase::ScintillaBytes ExsciScintilla::styleText(const QList<ExsciStyledText> &styled_text, char **styles, int style_offset)
{
    QString text;
    int i;
    ...
    {
        const ExsciStyledText &st = styled_text[i];
...
int ExsciScintilla::mapModifiers(int modifiers)
{
    int state = 0;
...
bool ExsciScintilla::event(QEvent *e)
{
    if (e->type() == QEvent::ShortcutOverride && !isReadOnly())
    {
        ...
        if (ke->key())
        {
            ...
            ExsciCommand *cmd = stdCmds->boundTo(ke->key() | (ke->modifiers() & ~Qt::KeypadModifier));
    ...
    return ExsciScintillaBase::event(e);
}


// Re-implemented to handle chenges to the enabled state.
void ExsciScintilla::changeEvent(QEvent *e)
{
    ExsciScintillaBase::changeEvent(e);
...
void ExsciScintilla::setEnabledColors(int style, QColor &fore, QColor &back)
{
    if (isEnabled())
    {
...
void ExsciScintilla::contextMenuEvent(QContextMenuEvent *e)
{
...
QMenu *ExsciScintilla::createStandardContextMenu()
{
    ...
    if (!read_only)
    {
        ...
        set_shortcut(action, ExsciCommand::Undo);
        action->setEnabled(isUndoAvailable());

        action = menu->addAction(tr("&Redo"), this, SLOT(redo()));
        set_shortcut(action, ExsciCommand::Redo);
        action->setEnabled(isRedoAvailable());

        menu->addSeparator();

        action = menu->addAction(tr("Cu&t"), this, SLOT(cut()));
        set_shortcut(action, ExsciCommand::SelectionCut);
        action->setEnabled(has_selection);
    }

    action = menu->addAction(tr("&Copy"), this, SLOT(copy()));
    set_shortcut(action, ExsciCommand::SelectionCopy);
    action->setEnabled(has_selection);

    if (!read_only)
    {
        action = menu->addAction(tr("&Paste"), this, SLOT(paste()));
        set_shortcut(action, ExsciCommand::Paste);
    ...
    set_shortcut(action, ExsciCommand::SelectAll);
...
void ExsciScintilla::set_shortcut(QAction *action, ExsciCommand::Command cmd_id) const
{
    ExsciCommand *cmd = stdCmds->find(cmd_id);
...
void ExsciScintilla::delete_selection()
{
...
void ExsciScintilla::setScrollWidth(int pixelWidth)
{
    SendScintilla(SCI_SETSCROLLWIDTH, pixelWidth);
}

// Get the scroll width.
int ExsciScintilla::scrollWidth() const
{
...
void ExsciScintilla::setScrollWidthTracking(bool enabled)
{
...
bool ExsciScintilla::scrollWidthTracking() const
{
