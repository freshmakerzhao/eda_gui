// This module implements the ExsciLexerIDL class.



#include "Exsci/exscilexeridl.h"
...
// The ctor.
ExsciLexerIDL::ExsciLexerIDL(QObject *parent)
...
// The dtor.
ExsciLexerIDL::~ExsciLexerIDL()
{
}


// Returns the language name.
const char *ExsciLexerIDL::language() const
{
    return "IDL";
}
...
QColor ExsciLexerIDL::defaultColor(int style) const
{
    if (style == UUID)
...
const char *ExsciLexerIDL::keywords(int set) const
{
    if (set != 1)
        return 0;
...
QString ExsciLexerIDL::description(int style) const
{
    if (style == UUID)
        return tr("UUID");
