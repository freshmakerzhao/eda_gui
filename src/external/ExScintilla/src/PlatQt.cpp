...
static ExsciSciPopup *PMenu(MenuID mid)
{
    return reinterpret_cast<ExsciSciPopup *>(mid);
}
...
#warning "Correct handling of QFont metrics requires Qt v4.7.0 or later"
    ...
    else
    {
        ...
        else if (fp.weight <= ExsciScintillaBase::SC_WEIGHT_NORMAL)
...
{
    Destroy();
    mid = new ExsciSciPopup();
}

void Menu::Destroy()
{
    ExsciSciPopup *m = PMenu(mid);
