...
static XsciSciPopup *PMenu(MenuID mid)
{
    return reinterpret_cast<XsciSciPopup *>(mid);
}
...
#warning "Correct handling of QFont metrics requires Qt v4.7.0 or later"
    ...
    else
    {
        ...
        else if (fp.weight <= XsciScintillaBase::SC_WEIGHT_NORMAL)
...
{
    Destroy();
    mid = new XsciSciPopup();
}

void Menu::Destroy()
{
    XsciSciPopup *m = PMenu(mid);
