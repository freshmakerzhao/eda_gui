...
// The ctor.
XsciScintillaQt::XsciScintillaQt(XsciScintillaBase *qsb_)
...
// The dtor.
XsciScintillaQt::~XsciScintillaQt()
{ 
    Finalise();
}
...
void XsciScintillaQt::Initialise()
{
...
// Start a drag.
void XsciScintillaQt::StartDrag()
{
...
sptr_t XsciScintillaQt::WndProc(unsigned int iMessage, uptr_t wParam,
        sptr_t lParam)
{
    switch (iMessage)
    {
...
// Windows nonsense.
sptr_t XsciScintillaQt::DefWndProc(unsigned int, uptr_t, sptr_t)
{
    return 0;
}
...
void XsciScintillaQt::SetMouseCapture(bool on)
{
...
bool XsciScintillaQt::HaveMouseCapture()
{
...
void XsciScintillaQt::SetVerticalScrollPos()
{
...
void XsciScintillaQt::SetHorizontalScrollPos()
{
...
bool XsciScintillaQt::ModifyScrollBars(Sci::Line nMax, Sci::Line nPage)
{
    bool modified = false;
    QScrollBar *sb;
...
void XsciScintillaQt::ReconfigureScrollBars()
{
...
void XsciScintillaQt::NotifyChange()
{
...
void XsciScintillaQt::NotifyParent(SCNotification scn)
{
    ...
    case SCN_MODIFIED:
        {
            ...
            if ((scn.modificationType & SC_MOD_INSERTTEXT) != 0)
                XsciAccessibleScintillaBase::textInserted(qsb, scn.position,
                        scn.text, scn.length);
            else if ((scn.modificationType & SC_MOD_DELETETEXT) != 0)
                XsciAccessibleScintillaBase::textDeleted(qsb, scn.position,
                        scn.text, scn.length);
#endif
    ...
    case SCN_UPDATEUI:
#if !defined(QT_NO_ACCESSIBILITY)
        XsciAccessibleScintillaBase::updated(qsb);
#endif
...
QMimeData *XsciScintillaQt::mimeSelection(
...
void XsciScintillaQt::CopyToClipboard(
...
void XsciScintillaQt::pasteFromClipboard(QClipboard::Mode mode)
{
    int len;
    const char *s;
    bool rectangular;
...
void XsciScintillaQt::CreateCallTipWindow(Scintilla::PRectangle rc)
{
    if (!ct.wCallTip.Created())
        ct.wCallTip = new XsciSciCallTip(qsb, this);

    XsciSciCallTip *w = reinterpret_cast<XsciSciCallTip *>(ct.wCallTip.GetID());
...
void XsciScintillaQt::AddToPopUp(const char *label, int cmd, bool enabled)
{
    XsciSciPopup *pm = static_cast<XsciSciPopup *>(popup.GetID());
...
void XsciScintillaQt::ClaimSelection()
{
...
#if !defined(QT_NO_ACCESSIBILITY)
    XsciAccessibleScintillaBase::selectionChanged(qsb, isSel);
#endif
...
void XsciScintillaQt::onSelectionChanged()
{
...
sptr_t XsciScintillaQt::DirectFunction(XsciScintillaQt *sciThis, unsigned int iMessage,
...
void XsciScintillaQt::paintEvent(QPaintEvent *e)
{
...
void XsciScintillaQt::timerEvent(QTimerEvent *e)
{
    for (int i = 0; i <= static_cast<int>(tickPlatform); ++i)
...
bool XsciScintillaQt::FineTickerAvailable()
{
    return true;
}
...
void XsciScintillaQt::FineTickerCancel(TickReason reason)
{
...
bool XsciScintillaQt::FineTickerRunning(TickReason reason)
{
...
void XsciScintillaQt::FineTickerStart(TickReason reason, int ms, int)
{
    int &ticker = timers[static_cast<int>(reason)];
...
bool XsciScintillaQt::SetIdle(bool on)
{
    if (on)
    {
        if (!idler.state)
