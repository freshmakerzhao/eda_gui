// This module implements the XsciMacro class.
 


#include "Xsci/xscimacro.h"
...
// The ctor.
XsciMacro::XsciMacro(XsciScintilla *parent)
...
XsciMacro::XsciMacro(const QString &asc, XsciScintilla *parent)
    : QObject(parent), qsci(parent)
{
    load(asc);
}
...
bool XsciMacro::load(const QString &asc)
{
    bool ok = true;
...
void XsciMacro::startRecording()
{
    ...
    qsci->SendScintilla(XsciScintillaBase::SCI_STARTRECORD);
}


// End recording.
void XsciMacro::endRecording()
{
    if (!qsci)
        return;

    qsci->SendScintilla(XsciScintillaBase::SCI_STOPRECORD);
    qsci->disconnect(this);
}


// Record a command.
void XsciMacro::record(unsigned int msg, unsigned long wParam, void *lParam)
{
    Macro m;
    ...
    switch (msg)
    {
    case XsciScintillaBase::SCI_ADDTEXT:
        m.text = QByteArray(reinterpret_cast<const char *>(lParam), wParam);
        break;

    case XsciScintillaBase::SCI_REPLACESEL:
        if (!macro.isEmpty() && macro.last().msg == XsciScintillaBase::SCI_REPLACESEL)
        {
    ...
    case XsciScintillaBase::SCI_INSERTTEXT:
    case XsciScintillaBase::SCI_APPENDTEXT:
    case XsciScintillaBase::SCI_SEARCHNEXT:
    case XsciScintillaBase::SCI_SEARCHPREV:
