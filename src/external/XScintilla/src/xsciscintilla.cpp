...
// The ctor.
XsciScintilla::XsciScintilla(QWidget *parent)
    : XsciScintillaBase(parent),
      ...
      use_single(AcusNever), explicit_fillups(""), fillups_enabled(false)
{
    ...
    stdCmds = new XsciCommandSet(this);
...
QColor XsciScintilla::color() const
{
...
// Set the text colour.
void XsciScintilla::setColor(const QColor &c)
{
    if (lex.isNull())
    {
...
bool XsciScintilla::overwriteMode() const
{
...
void XsciScintilla::setOverwriteMode(bool overwrite)
{
...
QColor XsciScintilla::paper() const
{
...
void XsciScintilla::setPaper(const QColor &c)
{
    if (lex.isNull())
    {
        // Assume style 0 applies to everything so that we don't need to use
...
// Set the default font.
void XsciScintilla::setFont(const QFont &f)
{
    if (lex.isNull())
    {
        // Assume style 0 applies to everything so that we don't need to use
...
void XsciScintilla::setAutoIndent(bool autoindent)
{
    autoInd = autoindent;
}
...
void XsciScintilla::setBraceMatching(BraceMatch bm)
{
    braceMode = bm;
}
...
void XsciScintilla::handleCharAdded(int ch)
{
...
bool XsciScintilla::isCallTipActive() const
{
...
void XsciScintilla::callTip()
{
    XsciAbstractAPIs *apis = lex->apis();
...
void XsciScintilla::handleCallTipClick(int dir)
{
    int nr_entries = ct_entries.count();
...
int XsciScintilla::adjustedCallTipPosition(int ctshift) const
{
    int ct = ctPos;
...
// of the context.
QStringList XsciScintilla::apiContext(int pos, int &context_start,
        int &last_word_start)
{
    enum {
        Either,
...
bool XsciScintilla::getSeparator(int &pos) const
{
    int opos = pos;
...
QString XsciScintilla::getWord(int &pos) const
{
    QString word;
    bool numeric = true;
    char ch;
...
char XsciScintilla::getCharacter(int &pos) const
{
    if (pos <= 0)
        return '\0';
...
bool XsciScintilla::isStartChar(char ch) const
{
    QString s = QChar(ch);
...
void XsciScintilla::startAutoCompletion(AutoCompletionSource acs,
...
void XsciScintilla::maintainIndentation(char ch, long pos)
{
    if (ch != '\r' && ch != '\n')
        return;
...
void XsciScintilla::autoIndentation(char ch, long pos)
{
    int curr_line = SendScintilla(SCI_LINEFROMPOSITION, pos);
...
void XsciScintilla::autoIndentLine(long pos, int line, int indent)
{
    if (indent < 0)
        return;
...
// significant before).
int XsciScintilla::blockIndent(int line)
{
    if (line < 0)
        return 0;
...
bool XsciScintilla::rangeIsWhitespace(long spos, long epos)
{
    while (spos < epos)
    {
...
XsciScintilla::IndentState XsciScintilla::getIndentState(int line)
{
    IndentState istate;
...
int XsciScintilla::findStyledWord(const char *text, int style,
        const char *words)
{
    if (!words)
        return -1;
...
bool XsciScintilla::isUtf8() const
{
...
// Set the code page.
void XsciScintilla::setUtf8(bool cp)
{
...
XsciScintilla::EolMode XsciScintilla::eolMode() const
{
...
void XsciScintilla::setEolMode(EolMode mode)
{
...
void XsciScintilla::convertEols(EolMode mode)
{
...
// Add an edge column.
void XsciScintilla::addEdgeColumn(int colnr, const QColor &col)
{
...
void XsciScintilla::clearEdgeColumns()
{
...
QColor XsciScintilla::edgeColor() const
{
...
// Set the edge colour.
void XsciScintilla::setEdgeColor(const QColor &col)
{
...
int XsciScintilla::edgeColumn() const
{
...
// Set the edge column.
void XsciScintilla::setEdgeColumn(int colnr)
{
...
XsciScintilla::EdgeMode XsciScintilla::edgeMode() const
{
...
// Set the edge mode.
void XsciScintilla::setEdgeMode(EdgeMode mode)
{
...
bool XsciScintilla::eolVisibility() const
{
...
void XsciScintilla::setEolVisibility(bool visible)
{
...
int XsciScintilla::extraAscent() const
{
...
void XsciScintilla::setExtraAscent(int extra)
{
...
int XsciScintilla::extraDescent() const
{
...
void XsciScintilla::setExtraDescent(int extra)
{
...
int XsciScintilla::whitespaceSize() const
{
...
void XsciScintilla::setWhitespaceSize(int size)
{
...
void XsciScintilla::setWhitespaceBackgroundColor(const QColor &col)
{
...
void XsciScintilla::setWhitespaceForegroundColor(const QColor &col)
{
...
XsciScintilla::WhitespaceVisibility XsciScintilla::whitespaceVisibility() const
{
...
void XsciScintilla::setWhitespaceVisibility(WhitespaceVisibility mode)
{
...
XsciScintilla::TabDrawMode XsciScintilla::tabDrawMode() const
{
...
void XsciScintilla::setTabDrawMode(TabDrawMode mode)
{
...
XsciScintilla::WrapMode XsciScintilla::wrapMode() const
{
...
void XsciScintilla::setWrapMode(WrapMode mode)
{
...
XsciScintilla::WrapIndentMode XsciScintilla::wrapIndentMode() const
{
...
void XsciScintilla::setWrapIndentMode(WrapIndentMode mode)
{
...
void XsciScintilla::setWrapVisualFlags(WrapVisualFlag endFlag,
...
void XsciScintilla::setFolding(FoldStyle folding, int margin)
{
    fold = folding;
    foldmargin = margin;
...
void XsciScintilla::setFoldMarker(int marknr, int mark)
{
...
void XsciScintilla::foldClick(int lineClick, int bstate)
{
...
// SciTE.
void XsciScintilla::foldExpand(int &line, bool doExpand, bool force,
...
void XsciScintilla::foldAll(bool children)
{
    recolor();
...
void XsciScintilla::foldChanged(int line,int levelNow,int levelPrev)
{
...
void XsciScintilla::foldLine(int line)
{
...
QList<int> XsciScintilla::contractedFolds() const
{
    QList<int> folds;
...
void XsciScintilla::setContractedFolds(const QList<int> &folds)
{
...
void XsciScintilla::handleModified(int pos, int mtype, const char *text,
...
void XsciScintilla::zoomIn(int range)
{
...
void XsciScintilla::zoomOut(int range)
{
    zoomTo(SendScintilla(SCI_GETZOOM) - range);
}
...
void XsciScintilla::zoomTo(int size)
{
    if (size < -10)
        size = -10;
    else if (size > 20)
...
bool XsciScintilla::findFirst(const QString &expr, bool re, bool cs, bool wo,
...
bool XsciScintilla::findFirstInSelection(const QString &expr, bool re, bool cs,
...
void XsciScintilla::replace(const QString &replaceStr)
{
    if (findState.status == FindState::Idle)
        return;
...
bool XsciScintilla::isModified() const
{
...
void XsciScintilla::setModified(bool m)
{
    if (!m)
...
void XsciScintilla::handleIndicatorClick(int pos, int modifiers)
{
...
void XsciScintilla::handleIndicatorRelease(int pos, int modifiers)
{
    int state = mapModifiers(modifiers);
    int line, index;
...
void XsciScintilla::handleMarginClick(int pos, int modifiers, int margin)
{
    int state = mapModifiers(modifiers);
...
void XsciScintilla::handleMarginRightClick(int pos, int modifiers, int margin)
{
    int state = mapModifiers(modifiers);
    int line = SendScintilla(SCI_LINEFROMPOSITION, pos);
...
void XsciScintilla::handleSavePointReached()
{
...
void XsciScintilla::handleSavePointLeft()
{
...
void XsciScintilla::handleSelectionChanged(bool yes)
{
    selText = yes;
...
void XsciScintilla::getSelection(int *lineFrom, int *indexFrom, int *lineTo,
        int *indexTo) const
{
    if (selText)
    {
...
void XsciScintilla::setSelection(int lineFrom, int indexFrom, int lineTo,
        int indexTo)
{
...
void XsciScintilla::setSelectionBackgroundColor(const QColor &col)
{
    int alpha = col.alpha();
...
void XsciScintilla::setSelectionForegroundColor(const QColor &col)
{
...
void XsciScintilla::resetSelectionBackgroundColor()
{
...
void XsciScintilla::resetSelectionForegroundColor()
{
...
void XsciScintilla::setSelectionToEol(bool filled)
{
...
bool XsciScintilla::selectionToEol() const
{
...
void XsciScintilla::setCaretWidth(int width)
{
...
void XsciScintilla::setCaretLineFrameWidth(int width)
{
...
void XsciScintilla::setCaretForegroundColor(const QColor &col)
{
...
void XsciScintilla::setCaretLineBackgroundColor(const QColor &col)
{
    int alpha = col.alpha();
...
void XsciScintilla::setCaretLineVisible(bool enable)
{
...
void XsciScintilla::setHotspotBackgroundColor(const QColor &col)
{
...
void XsciScintilla::setHotspotForegroundColor(const QColor &col)
{
...
void XsciScintilla::resetHotspotBackgroundColor()
{
...
void XsciScintilla::resetHotspotForegroundColor()
{
...
void XsciScintilla::setHotspotUnderline(bool enable)
{
...
void XsciScintilla::setHotspotWrap(bool enable)
{
...
bool XsciScintilla::isReadOnly() const
{
...
void XsciScintilla::setReadOnly(bool ro)
{
...
void XsciScintilla::append(const QString &text)
{
    bool ro = ensureRW();
...
void XsciScintilla::insert(const QString &text)
{
    insertAtPos(text, -1);
}
...
void XsciScintilla::insertAt(const QString &text, int line, int index)
{
...
void XsciScintilla::insertAtPos(const QString &text, int pos)
{
    bool ro = ensureRW();
...
void XsciScintilla::beginUndoAction()
{
    SendScintilla(SCI_BEGINUNDOACTION);
}
...
void XsciScintilla::endUndoAction()
{
    SendScintilla(SCI_ENDUNDOACTION);
}
...
bool XsciScintilla::isRedoAvailable() const
{
...
bool XsciScintilla::isUndoAvailable() const
{
...
int XsciScintilla::lineAt(const QPoint &pos) const
{
...
int XsciScintilla::lineLength(int line) const
{
...
int XsciScintilla::length() const
{
...
void XsciScintilla::removeSelectedText()
{
...
void XsciScintilla::replaceSelectedText(const QString &text)
{
...
QString XsciScintilla::selectedText() const
{
    if (!selText)
        return QString();
...
QString XsciScintilla::text() const
{
    int buflen = length() + 1;
...
QString XsciScintilla::text(int line) const
{
...
QString XsciScintilla::text(int start, int end) const
{
...
QByteArray XsciScintilla::bytes(int start, int end) const
{
...
// Set the given text.
void XsciScintilla::setText(const QString &text)
{
    bool ro = ensureRW();
...
void XsciScintilla::getCursorPosition(int *line, int *index) const
{
...
void XsciScintilla::setCursorPosition(int line, int index)
{
...
void XsciScintilla::ensureCursorVisible()
{
...
void XsciScintilla::ensureLineVisible(int line)
{
...
void XsciScintilla::selectAll(bool select)
{
    if (select)
...
int XsciScintilla::indentation(int line) const
{
...
void XsciScintilla::setIndentation(int line, int indentation)
{
    SendScintilla(SCI_BEGINUNDOACTION);
...
// Indent a line.
void XsciScintilla::indent(int line)
{
...
// Unindent a line.
void XsciScintilla::unindent(int line)
{
...
int XsciScintilla::currentIndent() const
{
...
int XsciScintilla::indentWidth() const
{
...
bool XsciScintilla::indentationGuides() const
{
...
void XsciScintilla::setIndentationGuides(bool enable)
{
    int iv;
...
void XsciScintilla::setIndentationGuidesBackgroundColor(const QColor &col)
{
...
void XsciScintilla::setIndentationGuidesForegroundColor(const QColor &col)
{
...
int XsciScintilla::indentationWidth() const
{
...
void XsciScintilla::setIndentationWidth(int width)
{
...
int XsciScintilla::tabWidth() const
{
...
// Set the tab width.
void XsciScintilla::setTabWidth(int width)
{
...
bool XsciScintilla::backspaceUnindents() const
{
...
void XsciScintilla::setBackspaceUnindents(bool unindents)
{
...
bool XsciScintilla::tabIndents() const
{
...
void XsciScintilla::setTabIndents(bool indents)
{
...
bool XsciScintilla::indentationsUseTabs() const
{
...
void XsciScintilla::setIndentationsUseTabs(bool tabs)
{
...
int XsciScintilla::margins() const
{
...
void XsciScintilla::setMargins(int margins)
{
...
QColor XsciScintilla::marginBackgroundColor(int margin) const
{
...
void XsciScintilla::setMarginBackgroundColor(int margin, const QColor &col)
{
...
int XsciScintilla::marginOptions() const
{
...
void XsciScintilla::setMarginOptions(int options)
{
...
XsciScintilla::MarginType XsciScintilla::marginType(int margin) const
{
...
// Set the margin type.
void XsciScintilla::setMarginType(int margin, XsciScintilla::MarginType type)
{
...
// Clear margin text.
void XsciScintilla::clearMarginText(int line)
{
    if (line < 0)
...
// Annotate a line.
void XsciScintilla::setMarginText(int line, const QString &text, int style)
{
...
// Annotate a line.
void XsciScintilla::setMarginText(int line, const QString &text, const XsciStyle &style)
{
    style.apply(this);
...
// Annotate a line.
void XsciScintilla::setMarginText(int line, const XsciStyledText &text)
{
    text.apply(this);
...
// Annotate a line.
void XsciScintilla::setMarginText(int line, const QList<XsciStyledText> &text)
{
    char *styles;
...
bool XsciScintilla::marginLineNumbers(int margin) const
{
...
void XsciScintilla::setMarginLineNumbers(int margin, bool lnrs)
{
...
int XsciScintilla::marginMarkerMask(int margin) const
{
...
void XsciScintilla::setMarginMarkerMask(int margin,int mask)
{
...
bool XsciScintilla::marginSensitivity(int margin) const
{
...
void XsciScintilla::setMarginSensitivity(int margin,bool sens)
{
...
int XsciScintilla::marginWidth(int margin) const
{
...
void XsciScintilla::setMarginWidth(int margin, int width)
{
...
void XsciScintilla::setMarginWidth(int margin, const QString &s)
{
...
void XsciScintilla::setMarginsBackgroundColor(const QColor &col)
{
...
void XsciScintilla::setMarginsForegroundColor(const QColor &col)
{
...
void XsciScintilla::setMarginsFont(const QFont &f)
{
...
// Define an indicator.
int XsciScintilla::indicatorDefine(IndicatorStyle style, int indicatorNumber)
{
...
bool XsciScintilla::indicatorDrawUnder(int indicatorNumber) const
{
...
void XsciScintilla::setIndicatorDrawUnder(bool under, int indicatorNumber)
{
...
void XsciScintilla::setIndicatorForegroundColor(const QColor &col,
        int indicatorNumber)
{
    if (indicatorNumber < INDIC_IME)
    {
...
void XsciScintilla::setIndicatorHoverForegroundColor(const QColor &col,
        int indicatorNumber)
{
    if (indicatorNumber < INDIC_IME)
    {
...
void XsciScintilla::setIndicatorHoverStyle(IndicatorStyle style,
        int indicatorNumber)
{
    if (indicatorNumber < INDIC_IME)
    {
...
void XsciScintilla::setIndicatorOutlineColor(const QColor &col, int indicatorNumber)
{
    if (indicatorNumber < INDIC_IME)
    {
        int alpha = col.alpha();
...
void XsciScintilla::fillIndicatorRange(int lineFrom, int indexFrom,
...
void XsciScintilla::clearIndicatorRange(int lineFrom, int indexFrom,
        int lineTo, int indexTo, int indicatorNumber)
{
    if (indicatorNumber < INDIC_IME)
    {
...
int XsciScintilla::markerDefine(MarkerSymbol sym, int markerNumber)
{
...
int XsciScintilla::markerDefine(char ch, int markerNumber)
{
    checkMarker(markerNumber);
...
int XsciScintilla::markerDefine(const QPixmap &pm, int markerNumber)
{
    checkMarker(markerNumber);
...
int XsciScintilla::markerDefine(const QImage &im, int markerNumber)
{
    checkMarker(markerNumber);
...
int XsciScintilla::markerAdd(int linenr, int markerNumber)
{
...
unsigned XsciScintilla::markersAtLine(int linenr) const
{
...
void XsciScintilla::markerDelete(int linenr, int markerNumber)
{
...
void XsciScintilla::markerDeleteAll(int markerNumber)
{
    if (markerNumber <= MARKER_MAX)
    {
        if (markerNumber < 0)
...
void XsciScintilla::markerDeleteHandle(int mhandle)
{
...
int XsciScintilla::markerLine(int mhandle) const
{
...
int XsciScintilla::markerFindNext(int linenr, unsigned mask) const
{
...
int XsciScintilla::markerFindPrevious(int linenr, unsigned mask) const
{
...
void XsciScintilla::setMarkerBackgroundColor(const QColor &col, int markerNumber)
{
    if (markerNumber <= MARKER_MAX)
    {
        int alpha = col.alpha();
...
void XsciScintilla::setMarkerForegroundColor(const QColor &col, int markerNumber)
{
    if (markerNumber <= MARKER_MAX)
    {
        if (markerNumber < 0)
...
void XsciScintilla::checkMarker(int &markerNumber)
{
...
void XsciScintilla::checkIndicator(int &indicatorNumber)
{
...
void XsciScintilla::allocateId(int &id, unsigned &allocated, int min, int max)
{
    if (id >= 0)
    {
...
void XsciScintilla::resetFoldMarginColors()
{
...
void XsciScintilla::setFoldMarginColors(const QColor &fore, const QColor &back)
{
...
void XsciScintilla::setCallTipsBackgroundColor(const QColor &col)
{
...
void XsciScintilla::setCallTipsForegroundColor(const QColor &col)
{
...
void XsciScintilla::setCallTipsHighlightColor(const QColor &col)
{
...
void XsciScintilla::setMatchedBraceBackgroundColor(const QColor &col)
{
...
void XsciScintilla::setMatchedBraceForegroundColor(const QColor &col)
{
...
void XsciScintilla::setMatchedBraceIndicator(int indicatorNumber)
{
...
void XsciScintilla::resetMatchedBraceIndicator()
{
...
void XsciScintilla::setUnmatchedBraceBackgroundColor(const QColor &col)
{
...
void XsciScintilla::setUnmatchedBraceForegroundColor(const QColor &col)
{
...
void XsciScintilla::setUnmatchedBraceIndicator(int indicatorNumber)
{
...
void XsciScintilla::resetUnmatchedBraceIndicator()
{
...
// Detach any lexer.
void XsciScintilla::detachLexer()
{
    if (!lex.isNull())
    {
        lex->setEditor(0);
...
// Set the lexer.
void XsciScintilla::setLexer(XsciLexer *lexer)
{
    // Detach any current lexer.
    detachLexer();
...
void XsciScintilla::setLexerStyle(int style)
{
...
XsciLexer *XsciScintilla::lexer() const
{
    return lex;
}
...
void XsciScintilla::handleStyleColorChange(const QColor &c, int style)
{
...
void XsciScintilla::handleStyleEolFillChange(bool eolfill, int style)
{
...
void XsciScintilla::handleStyleFontChange(const QFont &f, int style)
{
...
void XsciScintilla::setStylesFont(const QFont &f, int style)
{
...
void XsciScintilla::handleStylePaperChange(const QColor &c, int style)
{
...
void XsciScintilla::handlePropertyChange(const char *prop, const char *val)
{
...
void XsciScintilla::handleUpdateUI(int)
{
...
long XsciScintilla::checkBrace(long pos, int brace_style, bool &colonMode)
{
    long brace_pos = -1;
    char ch = SendScintilla(SCI_GETCHARAT, pos);
...
// a pair of braces.
bool XsciScintilla::findMatchingBrace(long &brace, long &other, BraceMatch mode)
{
...
void XsciScintilla::moveToMatchingBrace()
{
...
void XsciScintilla::selectToMatchingBrace()
{
...
void XsciScintilla::gotoMatchingBrace(bool select)
{
    long braceAtCaret;
    long braceOpposite;
...
int XsciScintilla::positionFromLineIndex(int line, int index) const
{
...
void XsciScintilla::lineIndexFromPosition(int position, int *line, int *index) const
{
...
void XsciScintilla::setAutoCompletionSource(AutoCompletionSource source)
{
    acSource = source;
}
...
void XsciScintilla::setAutoCompletionThreshold(int thresh)
{
    acThresh = thresh;
}
...
void XsciScintilla::setAutoCompletionWordSeparators(const QStringList &separators)
{
    if (lex.isNull())
        wseps = separators;
}
...
void XsciScintilla::autoCompleteFromAll()
{
...
void XsciScintilla::autoCompleteFromAPIs()
{
...
void XsciScintilla::autoCompleteFromDocument()
{
...
bool XsciScintilla::isWordCharacter(char ch) const
{
...
const char *XsciScintilla::wordCharacters() const
{
    return wchars;
}
...
void XsciScintilla::recolor(int start, int end)
{
...
void XsciScintilla::registerImage(int id, const QPixmap &pm)
{
...
void XsciScintilla::registerImage(int id, const QImage &im)
{
    SendScintilla(SCI_RGBAIMAGESETHEIGHT, im.height());
    SendScintilla(SCI_RGBAIMAGESETWIDTH, im.width());
...
void XsciScintilla::clearRegisteredImages()
{
...
void XsciScintilla::setAutoCompletionFillupsEnabled(bool enable)
{
    const char *fillups;
...
bool XsciScintilla::autoCompletionFillupsEnabled() const
{
...
void XsciScintilla::setAutoCompletionFillups(const char *fillups)
{
...
void XsciScintilla::setAutoCompletionCaseSensitivity(bool cs)
{
...
bool XsciScintilla::autoCompletionCaseSensitivity() const
{
...
void XsciScintilla::setAutoCompletionReplaceWord(bool replace)
{
...
bool XsciScintilla::autoCompletionReplaceWord() const
{
...
void XsciScintilla::setAutoCompletionUseSingle(AutoCompletionUseSingle single)
{
    use_single = single;
}
...
XsciScintilla::AutoCompletionUseSingle XsciScintilla::autoCompletionUseSingle() const
{
    return use_single;
}
...
void XsciScintilla::setAutoCompletionShowSingle(bool single)
{
...
bool XsciScintilla::autoCompletionShowSingle() const
{
...
void XsciScintilla::setCallTipsPosition(CallTipsPosition position)
{
...
void XsciScintilla::setCallTipsStyle(CallTipsStyle style)
{
...
void XsciScintilla::setCallTipsVisible(int nr)
{
    maxCallTips = nr;
}
...
void XsciScintilla::setDocument(const XsciDocument &document)
{
...
int XsciScintilla::firstVisibleLine() const
{
...
void XsciScintilla::setFirstVisibleLine(int linenr)
{
...
int XsciScintilla::textHeight(int linenr) const
{
...
bool XsciScintilla::isListActive() const
{
...
void XsciScintilla::handleAutoCompletionSelection()
{
    if (!lex.isNull())
    {
        XsciAbstractAPIs *apis = lex->apis();
...
// Display a user list.
void XsciScintilla::showUserList(int id, const QStringList &list)
{
...
void XsciScintilla::handleUserListSelection(const char *text, int id)
{
...
bool XsciScintilla::caseSensitive() const
{
...
// user list.
bool XsciScintilla::isAutoCompletionList() const
{
...
bool XsciScintilla::read(QIODevice *io)
{
...
bool XsciScintilla::write(QIODevice *io) const
{
...
QString XsciScintilla::wordAtLineIndex(int line, int index) const
{
...
// Return the word at the given coordinates.
QString XsciScintilla::wordAtPoint(const QPoint &point) const
{
...
QString XsciScintilla::wordAtPosition(int position) const
{
    if (position < 0)
        return QString();
...
XsciScintilla::AnnotationDisplay XsciScintilla::annotationDisplay() const
{
...
void XsciScintilla::setAnnotationDisplay(XsciScintilla::AnnotationDisplay display)
{
...
// Clear annotations.
void XsciScintilla::clearAnnotations(int line)
{
    if (line >= 0)
...
// Annotate a line.
void XsciScintilla::annotate(int line, const QString &text, int style)
{
...
// Annotate a line.
void XsciScintilla::annotate(int line, const QString &text, const XsciStyle &style)
{
    style.apply(this);
...
// Annotate a line.
void XsciScintilla::annotate(int line, const XsciStyledText &text)
{
    text.apply(this);
...
// Annotate a line.
void XsciScintilla::annotate(int line, const QList<XsciStyledText> &text)
{
    char *styles;
    ScintillaBytes styled_text = styleText(text, &styles,
...
QString XsciScintilla::annotation(int line) const
{
...
XsciScintillaBase::ScintillaBytes XsciScintilla::styleText(const QList<XsciStyledText> &styled_text, char **styles, int style_offset)
{
    QString text;
    int i;
    ...
    {
        const XsciStyledText &st = styled_text[i];
...
int XsciScintilla::mapModifiers(int modifiers)
{
    int state = 0;
...
bool XsciScintilla::event(QEvent *e)
{
    if (e->type() == QEvent::ShortcutOverride && !isReadOnly())
    {
        ...
        if (ke->key())
        {
            ...
            XsciCommand *cmd = stdCmds->boundTo(ke->key() | (ke->modifiers() & ~Qt::KeypadModifier));
    ...
    return XsciScintillaBase::event(e);
}


// Re-implemented to zoom when the Control modifier is pressed.
void XsciScintilla::wheelEvent(QWheelEvent *e)
{
#if defined(Q_OS_MAC)
   ...
   {
       XsciScintillaBase::wheelEvent(e);
   }
}


// Re-implemented to handle chenges to the enabled state.
void XsciScintilla::changeEvent(QEvent *e)
{
    XsciScintillaBase::changeEvent(e);
...
void XsciScintilla::setEnabledColors(int style, QColor &fore, QColor &back)
{
    if (isEnabled())
    {
...
void XsciScintilla::contextMenuEvent(QContextMenuEvent *e)
{
...
QMenu *XsciScintilla::createStandardContextMenu()
{
    ...
    if (!read_only)
    {
        ...
        set_shortcut(action, XsciCommand::Undo);
        action->setEnabled(isUndoAvailable());

        action = menu->addAction(tr("&Redo"), this, SLOT(redo()));
        set_shortcut(action, XsciCommand::Redo);
        action->setEnabled(isRedoAvailable());

        menu->addSeparator();

        action = menu->addAction(tr("Cu&t"), this, SLOT(cut()));
        set_shortcut(action, XsciCommand::SelectionCut);
        action->setEnabled(has_selection);
    }

    action = menu->addAction(tr("&Copy"), this, SLOT(copy()));
    set_shortcut(action, XsciCommand::SelectionCopy);
    action->setEnabled(has_selection);

    if (!read_only)
    {
        action = menu->addAction(tr("&Paste"), this, SLOT(paste()));
        set_shortcut(action, XsciCommand::Paste);
    ...
    set_shortcut(action, XsciCommand::SelectAll);
...
void XsciScintilla::set_shortcut(QAction *action, XsciCommand::Command cmd_id) const
{
    XsciCommand *cmd = stdCmds->find(cmd_id);
...
void XsciScintilla::delete_selection()
{
...
void XsciScintilla::setScrollWidth(int pixelWidth)
{
    SendScintilla(SCI_SETSCROLLWIDTH, pixelWidth);
}

// Get the scroll width.
int XsciScintilla::scrollWidth() const
{
...
void XsciScintilla::setScrollWidthTracking(bool enabled)
{
...
bool XsciScintilla::scrollWidthTracking() const
{
