// This defines the interface to the XsciLexer class.
...
class XsciAbstractAPIs;
class XsciScintilla;


//! \brief The XsciLexer class is an abstract class used as a base for language
//! lexers.
//!
...
//!
//! XsciLexer provides convenience methods for saving and restoring user
//! preferences for fonts and colours.
//!
//! If you want to write a lexer for a new language then you can add it to the
//! underlying Scintilla code and implement a corresponding XsciLexer sub-class
//! to manage the different styles used.  Alternatively you can implement a
//! sub-class of XsciLexerCustom.
class XSCINTILLA_EXPORT XsciLexer : public QObject
{
    Q_OBJECT

public:
    //! Construct a XsciLexer with parent \a parent.  \a parent is typically
    //! the XsciScintilla instance.
    XsciLexer(QObject *parent = 0);

    //! Destroy the XSciLexer.
    virtual ~XsciLexer();
    ...
    //! Returns the identifier (i.e. a XsciScintillaBase::SCLEX_* value) of the
    //! lexer.  This is only used if lexer() returns 0.  The default
    //! implementation returns XsciScintillaBase::SCLEX_CONTAINER.
    //!
    //! \sa lexer()
    ...
    //! language is block structured, or XsciScintilla::AiMaintain if not.
    //!
    //! \sa setAutoIndentStyle(), XsciScintilla::AiMaintain,
    //! XsciScintilla::AiOpening, XsciScintilla::AiClosing
    int autoIndentStyle();
    ...
    //! Returns the XsciScintilla instance that the lexer is currently attached
    //! to or 0 if it is unattached.
    XsciScintilla *editor() const {return attached_editor;}

    //! The current set of APIs is set to \a apis.  If \a apis is 0 then any
    //! existing APIs for this lexer are removed.
    //!
    //! \sa apis()
    void setAPIs(XsciAbstractAPIs *apis);
    ...
    //! \internal Set the XsciScintilla instance that the lexer is attached to.
    ...
    //!
    //! \sa writeSettings(), XsciScintilla::setLexer()
...
public slots:
    ...
    //!
    //! \sa autoIndentStyle(), XsciScintilla::AiMaintain,
    //! XsciScintilla::AiOpening, XsciScintilla::AiClosing
...
private:
    ...
    QColor defColor;
    QColor defPaper;
    XsciAbstractAPIs *apiSet;
    XsciScintilla *attached_editor;

    void setStyleDefaults() const;
    StyleData &styleData(int style) const;

    XsciLexer(const XsciLexer &);
    XsciLexer &operator=(const XsciLexer &);
};
