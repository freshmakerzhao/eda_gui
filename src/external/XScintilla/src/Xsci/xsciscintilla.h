...
#ifndef XSCISCINTILLA_H
#define XSCISCINTILLA_H
...
class XsciCommandSet;
class XsciLexer;
class XsciStyle;
class XsciStyledText;
class XsciListBoxQt;


//! \brief The XsciScintilla class implements a higher level, more Qt-like,
//! API to the Scintilla editor widget.
//!
//! XsciScintilla implements methods, signals and slots similar to those found
...
//! auto-indenting and auto-completion than that provided by XsciScintillaBase.
class XSCINTILLA_EXPORT XsciScintilla : public XsciScintillaBase
{
    Q_OBJECT
...
public:
    ...
    //! Construct an empty XsciScintilla with parent \a parent.
    XsciScintilla(QWidget *parent = 0);

    //! Destroys the XsciScintilla instance.
    virtual ~XsciScintilla();
    ...
    //! style.
    void annotate(int line, const QString &text, const XsciStyle &style);

    //! Annotate the line \a line with the styled text \a text.
    void annotate(int line, const XsciStyledText &text);

    //! Annotate the line \a line with the list of styled text \a text.
    void annotate(int line, const QList<XsciStyledText> &text);
    ...
    //! is typically used by XsciLexerCustom::styleText().
    //!
    //! \sa text()
    ...
    //!
    //! \sa setDocument()
    XsciDocument document() const {return doc;}
    ...
    //! XScintilla uses the combination of a line number and a character index
    ...
    //!
    //! \sa setMarginMask(), XsciMarker, SCI_GETMARGINMASKN
    ...
    //!
    //! \sa clearRegisteredImages(), XsciLexer::apiLoad()
    ...
    //!
    //! \sa document()
    void setDocument(const XsciDocument &document);
    ...
    //! Set the margin text of line \a line with the text \a text using the
    //! style \a style.
    void setMarginText(int line, const QString &text, const XsciStyle &style);

    //! Set the margin text of line \a line with the styled text \a text.
    void setMarginText(int line, const XsciStyledText &text);

    //! Set the margin text of line \a line with the list of styled text \a
    //! text.
    void setMarginText(int line, const QList<XsciStyledText> &text);
    ...
    //! For performance, XScintilla does not measure the display width of the
    ...
    //! pixels assumed by XScintilla to \a pixelWidth.  The default value is
    //! 2000.
    //!
    //! \sa scrollWidth(), setScrollWidthTracking()
    ...
    XsciCommandSet *standardCommands() const {return stdCmds;}
    ...
    //! typically used by XsciLexerCustom::styleText().
    //!
    //! \sa bytes(), setText()
...
public slots:
    ...
    //!
    //! \sa lexer()
    virtual void setLexer(XsciLexer *lexer = 0);
    ...
    //!
    //! \sa marginMarkerMask(), XsciMarker, SCI_SETMARGINMASKN
...
private:
    ...
    ScintillaBytes styleText(const QList<XsciStyledText> &styled_text,
    ...
    QPointer<XsciLexer> lex;
    XsciCommandSet *stdCmds;
    XsciDocument doc;
    ...
    // The following allow XsciListBoxQt to distinguish between an
    // auto-completion list and a user list, and to return the full selection
    // of an auto-completion list.
    friend class XsciListBoxQt;

    QString acSelection;
    bool isAutoCompletionList() const;

    void set_shortcut(QAction *action, XsciCommand::Command cmd_id) const;

    XsciScintilla(const XsciScintilla &);
    XsciScintilla &operator=(const XsciScintilla &);
};
