...
#ifndef XSCISCINTILLABASE_H
#define XSCISCINTILLABASE_H
...
//! \brief The XsciScintillaBase class implements the Scintilla editor widget
...
//! editor.
//!
//! XsciScintillaBase is a port to Qt of Scintilla. It implements the standard
//! Scintilla API which consists of a number of messages each taking up to
//! two arguments.
//!
//! See XsciScintilla for the implementation of a higher level API that is more
//! consistent with the rest of the Qt toolkit.
class XSCINTILLA_EXPORT XsciScintillaBase : public QAbstractScrollArea
{
    Q_OBJECT
...
public:
    ...
    enum
    {
        ...
        //! SC_CURSORREVERSEARROW.  Note that, currently, XScintilla implements
    ...
    //! Construct an empty XsciScintillaBase with parent \a parent.
    explicit XsciScintillaBase(QWidget *parent = 0);

    //! Destroys the XsciScintillaBase instance.
    virtual ~XsciScintillaBase();

    //! Returns a pointer to a XsciScintillaBase instance, or 0 if there isn't
    //! one.  This can be used by the higher level API to send messages that
    //! aren't associated with a particular instance.
    static XsciScintillaBase *pool();
...
#define ScintillaBytesConstData(b)  (b).constData()
    ...
    //! \internal A helper for XsciScintilla::contextMenuEvent().
...
private:
    // This is needed to allow XsciScintillaQt to emit this class's signals.
    friend class XsciScintillaQt;
    ...
    XsciScintillaBase(const XsciScintillaBase &);
    XsciScintillaBase &operator=(const XsciScintillaBase &);
};
