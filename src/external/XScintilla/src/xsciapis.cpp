// This module implements the XsciAPIs class.
...
struct XsciAPIsPrepared
{
    ...
    QMap<QString, XsciAPIs::WordIndexList> wdict;
...
QStringList XsciAPIsPrepared::apiWords(int api_idx, const QStringList &wseps,
        bool strip_image) const
{
...
QString XsciAPIsPrepared::apiBaseName(const QString &api)
{
    QString base = api;
    int tail = base.indexOf('(');
...
class XsciAPIsWorker : public QThread
{
public:
    XsciAPIsWorker(XsciAPIs *apis);
    virtual ~XsciAPIsWorker();

    virtual void run();

    XsciAPIsPrepared *prepared;
...
XsciAPIsWorker::XsciAPIsWorker(XsciAPIs *apis)
...
void XsciAPIsWorker::run()
{
    ...
    for (int a = 0; a < prepared->raw_apis.count(); ++a)
    {
        ...
        for (int w = 0; w < words.count(); ++w)
        {
            ...
            XsciAPIs::WordIndexList wil = prepared->wdict[word];
            ...
            wil.append(XsciAPIs::WordIndex(a, w));
...
// The ctor.
XsciAPIs::XsciAPIs(XsciLexer *lexer)
    : XsciAbstractAPIs(lexer), worker(0), origin_len(0)
{
    prep = new XsciAPIsPrepared;
}
...
bool XsciAPIs::load(const QString &filename)
{
    QFile f(filename);
...
void XsciAPIs::add(const QString &entry)
{
    apis.append(entry);
}
...
void XsciAPIs::remove(const QString &entry)
{
...
// supplied context.
QStringList XsciAPIs::positionOrigin(const QStringList &context, QString &path)
{
...
bool XsciAPIs::originStartsWith(const QString &path, const QString &wsep)
{
...
void XsciAPIs::updateAutoCompletionList(const QStringList &context,
        QStringList &list)
{
    QString path;
    ...
    if (origin_len > 0)
    {
        ...
        {
            QString base = XsciAPIsPrepared::apiBaseName(*it);
...
const XsciAPIs::WordIndexList *XsciAPIs::wordIndexOf(const QString &word) const
{
    QString csword;
...
void XsciAPIs::lastCompleteWord(const QString &word, QStringList &with_context, bool &unambig)
{
    // Get the possible API entries if any.
...
void XsciAPIs::lastPartialWord(const QString &word, QStringList &with_context, bool &unambig)
{
    if (lexer()->caseSensitive())
    {
...
void XsciAPIs::autoCompletionSelected(const QString &selection)
{
...
void XsciAPIs::addAPIEntries(const WordIndexList &wl, bool complete,
...
QStringList XsciAPIs::callTips(const QStringList &context, int commas,
        XsciScintilla::CallTipsStyle style, QList<int> &shifts)
{
    ...
    if (origin_len > 0)
    {
        ...
        if (style == XsciScintilla::CallTipsContext)
        {
            shift = ctstart;
            ctstart = 0;
        }
    ...
    else
    {
        ...
        if (wil)
            for (int i = 0; i < wil->count(); ++i)
            {
                ...
                if (style == XsciScintilla::CallTipsNoContext)
                {
                    shifts << 0;
...
bool XsciAPIs::enoughCommas(const QString &s, int commas)
{
    int end = s.indexOf(')');
...
void XsciAPIs::prepare()
{
    // Handle the trivial case.
    if (worker)
        return;

    XsciAPIsPrepared *new_apis = new XsciAPIsPrepared;
    new_apis->raw_apis = apis;

    worker = new XsciAPIsWorker(this);
...
void XsciAPIs::cancelPreparation()
{
    deleteWorker();
}
...
bool XsciAPIs::isPrepared(const QString &filename) const
{
...
bool XsciAPIs::loadPrepared(const QString &filename)
{
    QString pname = prepName(filename);
...
bool XsciAPIs::savePrepared(const QString &filename) const
{
...
QString XsciAPIs::defaultPreparedName() const
{
...
QString XsciAPIs::prepName(const QString &filename, bool mkpath) const
{
...
QStringList XsciAPIs::installedAPIFiles() const
{
