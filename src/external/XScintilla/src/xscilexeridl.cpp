// This module implements the XsciLexerIDL class.
 


#include "Xsci/xscilexeridl.h"
...
// The ctor.
XsciLexerIDL::XsciLexerIDL(QObject *parent)
    : XsciLexerCPP(parent)
{
}
...
// The dtor.
XsciLexerIDL::~XsciLexerIDL()
{
}


// Returns the language name.
const char *XsciLexerIDL::language() const
{
    return "IDL";
}
...
QColor XsciLexerIDL::defaultColor(int style) const
{
    if (style == UUID)
...
const char *XsciLexerIDL::keywords(int set) const
{
    if (set != 1)
        return 0;
...
QString XsciLexerIDL::description(int style) const
{
    if (style == UUID)
        return tr("UUID");
