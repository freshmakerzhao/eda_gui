...
FrameView::FrameView(const std::string& tileGridPath, const std::string& tileColorPathLocal, QString projectImplPath, FPGAOpenGLWidget3 *glwidget, QWidget *parent)
        :  QWidget(parent), scene(new QGraphicsScene(this))
{
    // viewer.setFPGAView(glwidget);
    // viewer.setFPGAView2(glwidget);
    viewer.setFPGAView3(glwidget);
   ...
   for (auto& cols : viewer.gridMatrix) {
       for (auto& item : cols) {
           ...
           for (SitesBlock* site:item->child_items) {
               connect(site, &Sites::SiteClicked, this, &FrameView::showSiteInfo);
           }
       }
   }
}
